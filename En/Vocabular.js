const voc = {
  "Разговор": {
    "Встреча, знакомство, этикет": [
      {
        ru: [
          "господин (обращение к мужчине)",
        ],
        en: [
          "Mr",
        ],
        fr: [
          "Monsieur",
        ],
        pl: [
          "pan",
        ],
      },
      {
        ru: [
          "барышня (обращение к незамужней девушке)",
        ],
        en: [
          "Miss",
        ],
        fr: [
          "Mademoiselle",
        ],
        pl: [
          "panna",
        ],
      },
      {
        ru: [
          "госпожа (обращение к замужней женщине)",
        ],
        en: [
          "Mrs",
        ],
        fr: [
          "Madame",
        ],
        pl: [
          "pani",
        ],
      },
      {
        ru: [
          "привет",
        ],
        pl: [
          "cześć",
        ],
      },
      {
        ru: [
          "пока",
          "до скорого",
        ],
        pl: [
          "cześć",
          "na razie",
        ],
      },
      {
        ru: [
          "а вы? (м.р.)",
        ],
        pl: [
          "a pan?",
        ],
      },
      {
        ru: [
          "а ты?",
        ],
        pl: [
          "a ty?",
        ],
      },
      {
        ru: [
          "очень приятно",
          "мне приятно",
        ],
        pl: [
          "bardzo mi miło",
          "miło mi",
        ],
      },
      {
        ru: [
          "Что это значит?",
        ],
        pl: [
          "co to znaczy?",
        ],
      },
      {
        ru: [
          "Ты знаешь?",
        ],
        pl: [
          "Czy wiesz?",
        ],
      },
      {
        ru: [
          "до свидания",
        ],
        pl: [
          "do widzenia",
        ],
      },
      {
        ru: [
          "Спокойной ночи",
          "доброй ночи",
        ],
        pl: [
          "dobranoc",
        ],
      },
      {
        ru: [
          "Добрый вечер",
        ],
        pl: [
          "dobry wieczór",
        ],
      },
      {
        ru: [
          "Добрый день",
          "здравствуйте",
        ],
        pl: [
          "dzień dobry",
        ],
      },
      {
        ru: [
          "спасибо",
        ],
        pl: [
          "dziękuję",
        ],
      },
      {
        ru: [
          "Где находится?",
        ],
        pl: [
          "gdzie jest?",
        ],
      },
      {
        ru: [
          "Где он живет?",
        ],
        pl: [
          "Gdzie on mieszka?",
        ],
      },
      {
        ru: [
          "Как вас зовут? (ж.р. важно имя)",
        ],
        pl: [
          "Jak ma pani na imię?",
        ],
      },
      {
        ru: [
          "Как тебя зовут? (важно имя)",
        ],
        pl: [
          "Jak masz na imię?",
        ],
      },
      {
        ru: [
          "Как дела? (на ты)",
        ],
        pl: [
          "Jak się masz?",
        ],
      }, {
        ru: [
          "Как дела? (на вы.м.р.)",
        ],
        pl: [
          "Jak się pan ma?",
        ],
      },
      {
        ru: [
          "Как тебя зовут? (важна фамилия)",
        ],
        pl: [
          "Jak się nazywasz?",
        ],
      },
      {
        ru: [
          "как вас зовут? (ж.р. Важна фамилия)",
        ],
        pl: [
          "Jak się pani nazywa?",
        ],
      },
      {
        ru: [
          "Какой ваш номер телефона? (м.р.)",
        ],
        pl: [
          "Jaki numer telefonu ma pan?",
        ],
      },
      {
        ru: [
          "Какой у него адрес?",
        ],
        pl: [
          "Jaki on ma adres?",
        ],
      },
      {
        ru: [
          "я Ева",
        ],
        pl: [
          "Jestem Ewa",
        ],
      },
      {
        ru: [
          "Я из Польши",
        ],
        pl: [
          "Jestem z Polski",
        ],
      },
      {
        ru: [
          "меня зовут Ева",
        ],
        pl: [
          "Mam na imię Ewa",
        ],
      },
      {
        ru: [
          "моя фамилия Ковальский",
        ],
        pl: [
          "Nazywam się Kowalski",
        ],
      },
      {
        ru: [
          "ничего страшного",
        ],
        pl: [
          "nic nie szkodzi",
        ],
      },
      {
        ru: [
          "не понимаю",
        ],
        pl: [
          "nie rozumiem",
        ],
      },
      {
        ru: [
          "не знаю",
          "я не знаю",
        ],
        pl: [
          "nie wiem",
        ],
      },
      {
        ru: [
          "Он из Польши",
        ],
        pl: [
          "On jest z Polski",
        ],
      },
      {
        ru: [
          "Он живет в Кракове",
        ],
        pl: [
          "On mieszka w Krakowie",
        ],
      },
      {
        ru: [
          "пожалуйста",
        ],
        pl: [
          "proszę",
        ],
      },
      {
        ru: [
          "прошу говорить медленнее",
          "пожалуйста говорите медленнее",
          "говорите медленнее пожалуйста",
        ],
        pl: [
          "proszę mówić wolniej",
        ],
      },
      {
        ru: [
          "прошу написать",
          "напишите пожалуйста",
        ],
        pl: [
          "proszę napisać",
        ],
      },
      {
        ru: [
          "прошу послушать",
          "послушайте пожалуйста",
        ],
        pl: [
          "proszę posłuchać",
        ],
      },
      {
        ru: [
          "прошу повторить",
          "повторите пожалуйста",
        ],
        pl: [
          "proszę powtórzyć",
        ],
      },
      {
        ru: [
          "прошу прочитать",
          "прочитайте пожалуйста",
        ],
        pl: [
          "proszę przeczytać",
        ],
      },
      {
        ru: [
          "прошу расположить",
          "расположите пожалуйста",
        ],
        pl: [
          "proszę ułożyć",
        ],
      },
      {
        ru: [
          "прошу заполнить",
          "пожалуйста заполните",
        ],
        pl: [
          "proszę uzupełnić",
        ],
      },
      {
        ru: [
          "извините",
          "прошу прощения",
        ],
        pl: [
          "przepraszam",
        ],
      },
      {
        ru: [
          "Откуда пан Ковальски?",
        ],
        pl: [
          "Skąd jest pan Kowalski?",
        ],
      },
      {
        ru: [
          "Откуда он?",
        ],
        pl: [
          "Skąd on jest?",
        ],
      },
      {
        ru: [
          "Откуда они? (м.р.)",
        ],
        pl: [
          "Skąd oni są?",
        ],
      },
      {
        ru: [
          "Откуда вы? (ед. число)",
        ],
        pl: [
          "Skąd pan jest? (м.р.)",
        ],
      },
      {
        ru: [
          "есть вопрос",
          "у меня вопрос",
          "у меня есть вопрос",
        ],
        pl: [
          "mam pytanie",
        ],
      },
      {
        ru: [
          "пан (уважительное обращение к мужчине)",
        ],
        pl: [
          "pan",
        ],
      },
      {
        ru: [
          "пани (уважительное обращение к женщине)",
        ],
        pl: [
          "pani",
        ],
      },
      {
        ru: [
          "вы (обращение к смешанной группе)",
        ],
        pl: [
          "państwo",
        ],
      },
      {
        ru: [
          "Кто это?",
        ],
        pl: [
          "kto to jest?",
        ],
      },
    ],










    "Разговор": [
      {
        ru: [
          "диалог",
        ],
        pl: [
          "dialog",
        ],
      },
      {
        ru: [
          "ответ",
        ],
        pl: [
          "odpowiedź",
        ],
      },
      {
        ru: [
          "информация",
        ],
        pl: [
          "informacja",
        ],
      },
      {
        ru: [
          "проблема",
        ],
        pl: [
          "problem",
        ],
      },
      {
        ru: [
          "представление",
        ],
        pl: [
          "przedstawianie się",
        ],
      },
      {
        ru: [
          "пример",
        ],
        pl: [
          "przykład",
        ],
      },
      {
        ru: [
          "вопрос",
        ],
        pl: [
          "pytanie",
        ],
      },
      {
        ru: [
          "конфликт",
        ],
        en: [
          "conflict",
        ],
      },
      {
        ru: [
          "шуметь",
        ],
        en: [
          "make a noise",
        ],
      },
    ],
    
    
    
    
    
    
    
    
    
    
    "Слова-связки": {
      "Добавление информации": [
        {
          ru: [
            "вместе с",
            "наряду с",
            "помимо",
            "кроме",
            "кроме того",
            "более того",
          ],
          en: [
            "along with",
            "furthermore",
            "moreover",
            "besides",
            "what's more",
            "apart from",
          ],
        },
        {
          ru: [
            "также",
            "тоже",
          ],
          en: [
            "also",
            "too",
          ],
        },
        {
          ru: [
            "и",
            "а также",
          ],
          en: [
            "and",
            "as well as",
          ],
        },
        {
          ru: [
            "к тому же",
            "вдобавок",
          ],
          en: [
            "in addition (to)",
          ],
        },
      ],
      "Контрастные идеи": [
        {
          ru: [
            "хотя",
            "несмотря на то что",
          ],
          en: [
            "although",
            "though",
          ],
        },
        {
          ru: [
            "даже если",
          ],
          en: [
            "even though",
          ],
        },
        {
          ru: [
            "но",
          ],
          en: [
            "but",
          ],
        },
        {
          ru: [
            "несмотря на",
            "вопреки",
          ],
          en: [
            "despite",
            "in spite of",
          ],
        },
        {
          ru: [
            "несмотря на тот факт что",
            "не смотря на то что",
          ],
          en: [
            "despite the fact that",
            "in spite of the fact that",
          ],
        },
        {
          ru: [
            "однако",
            "тем не менее",
            "хотя",
          ],
          en: [
            "however",
            "nevertheless",
            "nonetheless",
          ],
        },
        {
          ru: [
            "на практике",
          ],
          en: [
            "in practice",
          ],
        },
        {
          ru: [
            "теоретически",
            "в теории",
          ],
          en: [
            "in theory",
          ],
        },
        {
          ru: [
            "в отличие от",
          ],
          en: [
            "unlike",
          ],
        },
        {
          ru: [
            "тем временем",
            "между тем",
            "в это время",
            "в то время как",
            "тогда как",
          ],
          en: [
            "meanwhile",
            "whereas",
            "while",
          ],
        },
      ],
      "Причина": [
        {
          ru: [
            "так как",
            "из-за",
            "потому что",
            "благодаря",
            "поскольку",
          ],
          en: [
            "as",
            "due to",
            "because of",
            "because",
            "since",
          ],
        },
        {
          ru: [
            "из-за того факта что",
            "из-за того что",
            "благодаря тому факту что",
          ],
          en: [
            "due to the fact that",
          ],
        },
      ],
      "Результат": [
        {
          ru: [
            "в результате",
            "в итоге",
          ],
          en: [
            "as a result",
            "in summary",
          ],
        },
        {
          ru: [
            "следовательно",
            "как следствие",
          ],
          pl: [
            "consequently",
            "therefore",
            "so",
            "thus",
          ],
        },
        {
          ru: [
            "это означает что",
            "Это подразумевает что",
          ],
          en: [
            "this means that",
          ],
        },
        {
          ru: [
            "в двух словах",
          ],
          en: [
            "in a nutshell",
          ],
        },
        {
          ru: [
            "короче говоря",
            "вкратце",
          ],
          en: [
            "in short",
            "in brief",
          ],
        },
        {
          ru: [
            "кратко подводя итог",
          ],
          en: [
            "to sum up briefly",
          ],
        },
        {
          ru: [
            "мы можем заключить что",
          ],
          en: [
            "we can conclude that",
          ],
        },
        {
          ru: [
            "подводя итог",
            "чтобы подвести итог",
          ],
          en: [
            "to summarise",
          ],
        },
      ],
      "Сопоставление": [
        {
          ru: [
            "соответственно",
            "соответствующим образом",
          ],
          en: [
            "correspondingly",
          ],
        },
        {
          ru: [
            "в равной степени",
            "одинаково",
          ],
          en: [
            "equally",
          ],
        },
        {
          ru: [
            "таким же образом",
            "так же",
            "аналогично",
          ],
          en: [
            "in the same way",
            "likewise",
            "similarly",
          ],
        },
      ],
      "Идеи последовательности": [
        {
          ru: [
            "наконец",
            "в конце концов",
            "в заключение",
          ],
          en: [
            "finally",
            "lastly",
            "in conclusion",
          ],
        },
        {
          ru: [
            "во-первых",
          ],
          en: [
            "firstly",
            "the first point is",
          ],
        },
        {
          ru: [
            "во-вторых",
          ],
          en: [
            "secondly",
          ],
        },
        {
          ru: [
            "следующее",
          ],
          en: [
            "the following",
          ],
        },
        {
          ru: [
            "бывший",
            "прежний",
          ],
          en: [
            "the former",
          ],
        },
        {
          ru: [
            "последний",
            "окончательный",
          ],
          en: [
            "the latter",
            "last",
            "final",
            "latest",
          ],
        },
      ],
      "Примеры": [
        {
          ru: [
            "например",
          ],
          en: [
            "for example",
            "for instance",
          ],
        },
        {
          ru: [
            "а именно",
          ],
          en: [
            "namely",
          ],
        },
      ],
    },










    "Термины общего характера": [
      {
        ru: [
          "проблема",
          "вопрос",
        ],
        en: [
          "problem",
          "issue",
        ],
      },
    ],
  },
  









  "Человек": {
    "Личные данные": [
      {
        ru: [
          "быть из",
          "быть родом из",
        ],
        en: [
          "be from",
        ],
        fr: [
          "être d'origine de",
        ],
      },
      {
        ru: [
          "возраст",
        ],
        en: [
          "age",
          "stages",
        ],
        fr: [
          "des stades",
        ],
        pl: [
          "etapy rozwoju",
        ],
      },
      {
        ru: [
          "имя",
        ],
        en: [
          "name",
        ],
        pl: [
          "imię",
        ],
      },
      {
        ru: [
          "фамилия",
        ],
        en: [
          "surname",
          "last name",
        ],
        pl: [
          "nazwisko",
        ],
      },
      {
        ru: [
          "название",
        ],
        pl: [
          "nazwa",
        ],
      },
      {
        ru: [
          "идентификационный номер",
        ],
        pl: [
          "NIP",
        ],
      },
      {
        ru: [
          "почтовый индекс",
        ],
        pl: [
          "kod pocztowy",
        ],
      },
      {
        ru: [
          "номер мобилки",
          "номер мобильного",
        ],
        pl: [
          "numer komórki",
        ],
      },
      {
        ru: [
          "номер телефона",
        ],
        en: [
          "phone number",
        ],
        pl: [
          "numer telefonu",
        ],
      },
      {
        ru: [
          "паспорт",
        ],
        en: [
          "passport",
        ],
        pl: [
          "paszport",
        ],
      },
      {
        ru: [
          "регистрационный номер",
        ],
        pl: [
          "numer rejestracyjny",
        ],
      },
      {
        ru: [
          "адрес",
        ],
        en: [
          "address",
        ],
        pl: [
          "adres",
        ],
      }
    ],









    
    "Семья. Окружение": [
      /* Окружение */
      {
        ru: [
          "поколение",
        ],
        // en: [
        //   "generation",
        // ],
        fr: [
          "une génération",
        ],
        pl: [
          "pokolenie",
        ],
      },
      {
        ru: [
          "мужчина",
        ],
        // en: [
        //   "man",
        // ],
        fr: [
          "un homme",
        ],
        pl: [
          "mężczyzna",
        ],
      },
      {
        ru: [
          "женщина",
        ],
        // en: [
        //   "woman",
        // ],
        fr: [
          "une femme",
        ],
        pl: [
          "kobieta",
        ],
      },
      {
        ru: [
          "мальчик",
        ],
        // en: [
        //   "boy",
        // ],
        fr: [
          "un garçon",
        ],
        pl: [
          "chłopiec",
        ],
      },
      {
        ru: [
          "девочка",
          "девушка",
        ],
        // en: [
        //   "girl",
        // ],
        fr: [
          "une fille",
        ],
        pl: [
          "dziewczynka",
        ],
      },
      {
        ru: [
          "друг",
        ],
        // en: [
        //   "friend",
        // ],
        pl: [
          "kolega",
        ],
      },
      {
        ru: [
          "подруга",
          "подружка",
        ],
        // en: [
        //   "girlfriend",
        // ],
        pl: [
          "koleżanka",
        ],
      },
      {
        ru: [
          "персона",
          "человек",
        ],
        // en: [
        //   "person",
        //   "human",
        // ],
        pl: [
          "osoba",
        ],
      },
      
      
      
      
      
      
      
      
      
      
      /* Семья */
      {
        ru: [
          "семья",
        ],
        // en: [
        //   "family",
        // ],
        fr: [
          "une famille",
        ],
      },
      {
        ru: [
          "муж",
          "супруг",
        ],
        // en: [
        //   "husband",
        // ],
        fr: [
          "un mari",
        ],
        pl: [
          "mąż",
        ],
      },
      {
        ru: [
          "жена",
          "супруга",
        ],
        // en: [
        //   "wife",
        // ],
        fr: [
          "une femme",
        ],
        pl: [
          "żona",
        ],
      },
      {
        ru: [
          "отец",
        ],
        // en: [
        //   "father",
        // ],
        fr: [
          "un père",
        ],
        pl: [
          "ojciec",
        ],
      },
      {
        ru: [
          "папа",
          "папочка",
        ],
        // en: [
        //   "dad",
        //   "daddy",
        // ],
        fr: [
          "papa",
        ],
      },
      {
        ru: [
          "мать",
        ],
        // en: [
        //   "mother",
        // ],
        fr: [
          "une mère",
        ],
        pl: [
          "matka",
        ],
      },
      {
        ru: [
          "мама",
          "мамочка",
        ],
        // en: [
        //   "mum",
        //   "mummy",
        // ],
        fr: [
          "maman",
        ],
      },
      {
        ru: [
          "сын",
        ],
        // en: [
        //   "son",
        // ],
        fr: [
          "un fils",
        ],
        pl: [
          "syn"
        ],
      },
      {
        ru: [
          "дочь",
        ],
        // en: [
        //   "daughter",
        // ],
        fr: [
          "une fille",
        ],
        pl: [
          "córka",
        ],
      },
      {
        ru: [
          "брат",
        ],
        // en: [
        //   "brother",
        // ],
        fr: [
          "un frère",
        ],
        pl: [
          "brat",
        ],
      },
      {
        ru: [
          "сестра",
        ],
        // en: [
        //   "sister",
        // ],
        fr: [
          "une soeur",
        ],
        pl: [
          "siostra",
        ],
      },
      // {
      //   ru: [
      //     "узкий семейный круг",
      //     "близкие родственники",
      //     "ближайшие родственники",
      //   ],
      //   en: [
      //     "immediate family",
      //   ],
      // },
      // {
      //   ru: [
      //     "родной брат",
      //     "родная сестра",
      //   ],
      //   en: [
      //     "sibling",
      //   ],
      // },
      {
        ru: [
          "родители",
        ],
        // en: [
        //   "parents",
        // ],
        fr: [
          "des parents",
        ],
        pl: [
          "rodzice",
        ],
      },
      {
        ru: [
          "дедушка",
        ],
        // en: [
        //   "grandfather",
        // ],
        fr: [
          "un grand-père",
        ],
        pl: [
          "dziadek",
        ],
      },
      {
        ru: [
          "бабушка",
        ],
        // en: [
        //   "grandmother",
        // ],
        fr: [
          "une grand-mère",
        ],
        pl: [
          "babcia",
        ],
      },
      {
        ru: [
          "внук",
        ],
        // en: [
        //   "grandson",
        // ],
        fr: [
          "un petit-fils",
        ],
        pl: [
          "wnuk",
        ],
      },
      {
        ru: [
          "внучка",
        ],
        // en: [
        //   "granddaughter",
        // ],
        fr: [
          "une petite-fille",
        ],
        pl: [
          "wnuczka",
        ],
      },
      {
        ru: [
          "дедушка и бабушка",
        ],
        // en: [
        //   "grandparents",
        // ],
        fr: [
          "des grands-parents",
        ],
        pl: [
          "dziadkowie",
        ],
      },
      {
        ru: [
          "внуки",
        ],
        // en: [
        //   "grandchildren",
        // ],
        fr: [
          "des petits-enfants",
        ],
        pl: [
          "wnuki",
        ],
      },
      {
        ru: [
          "глава семьи",
        ],
        // en: [
        //   "head of the family",
        // ],
        fr: [
          "un chef de famille",
        ],
      },
      {
        ru: [
          "член семьи",
        ],
        // en: [
        //   "family member",
        //   "member of family",
        // ],
        fr: [
          "un membre de famille",
        ],
      },
      {
        ru: [
          "потомок",
        ],
        // en: [
        //   "descendant",
        // ],
        fr: [
          "un descendant",
          "une descendante",
        ],
      },
      {
        ru: [
          "предок",
        ],
        // en: [
        //   "ancestor",
        // ],
        fr: [
          "(un/une) ancêtre",
        ],
      },










      /* Родственники */
      {
        ru: [
          "быть родственником (кому-то)",
        ],
        // en: [
        //   "be related to (somebody)",
        // ],
        fr: [
          "être parent de",
        ],
      },
      {
        ru: [
          "генеалогическое древо",
          "семейное древо",
        ],
        // en: [
        //   "family tree",
        // ],
        fr: [
          "l'arbre généalogique",
        ],
      },
      {
        ru: [
          "родственники по рождению",
          "родственники по крови",
          "кровные родственники",
        ],
        // en: [
        //   "relatives by birth",
        // ],
        fr: [
          "des consanguins",
        ],
      },
      {
        ru: [
          "родственники по браку",
        ],
        // en: [
        //   "relatives by marriage",
        // ],
        fr: [
          "des parents selon le mariage",
        ],
      },
      {
        ru: [
          "родственники",
        ],
        // en: [
        //   "relatives",
        // ],
        fr: [
          "des parents",
        ],
        pl: [
          "krewni",
        ],
      },
      {
        ru: [
          "двоюродный брат",
          "двоюродная сестра",
          "кузен",
          "кузина",
        ],
        // en: [
        //   "cousin",
        // ],
        fr: [
          "un cousin",
        ],
        pl: [
          "kuzyn",
        ],
      },
      {
        ru: [
          "дядя",
        ],
        // en: [
        //   "uncle",
        // ],
        fr: [
          "un oncle",
        ],
        pl: [
          "wujek",
        ],
      },
      {
        ru: [
          "тетя",
          "тетка",
        ],
        // en: [
        //   "aunt",
        // ],
        fr: [
          "une tante",
        ],
        pl: [
          "ciotka",
        ],
      },
      {
        ru: [
          "племянник",
        ],
        // en: [
        //   "nephew",
        // ],
        fr: [
          "un neveu",
        ],
        pl: [
          "siostrzeniec",
        ],
      },
      {
        ru: [
          "племянница",
        ],
        // en: [
        //   "niece",
        // ],
        fr: [
          "une nièce",
        ],
        pl: [
          "siostrzenica",
        ],
      },
      {
        ru: [
          "свекор",
          "тесть",
        ],
        // en: [
        //   "father-in-law",
        // ],
        fr: [
          "un beau-père",
        ],
        pl: [
          "teść",
        ],
      },
      {
        ru: [
          "свекровь",
          "теща",
        ],
        // en: [
        //   "mother-in-law",
        // ],
        fr: [
          "une belle-mère",
        ],
        pl: [
          "teściowa",
        ],
      },
      {
        ru: [
          "шурин",
          "брат мужа",
          "брат сестры",
        ],
        // en: [
        //   "brother-in-law",
        // ],
        fr: [
          "un beau-frère",
        ],
        pl: [
          "szwagier",
        ],
      },
      {
        ru: [
          "золовка",
          "сестра мужа",
          "сестра жены",
        ],
        // en: [
        //   "sister-in-law",
        // ],
        fr: [
          "une belle-soeur",
        ],
        pl: [
          "szwagierka",
        ],
      },
      {
        ru: [
          "зять",
        ],
        // en: [
        //   "son-in-law",
        // ],
        fr: [
          "un gendre",
        ],
        pl: [
          "źięć",
        ],
      },
      {
        ru: [
          "невестка",
        ],
        // en: [
        //   "daughter-in-law",
        // ],
        fr: [
          "une belle-fille",
        ],
        pl: [
          "synowa",
        ],
      },
      {
        ru: [
          "отчим",
          "приемный отец",
        ],
        // en: [
        //   "stepfather",
        // ],
        fr: [
          "un beau-père",
        ],
        pl: [
          "ojczym",
        ],
      },
      {
        ru: [
          "мачеха",
          "приемная мать",
        ],
        // en: [
        //   "stepmother",
        // ],
        fr: [
          "une belle-mère",
        ],
        pl: [
          "macocha",
        ],
      },
      {
        ru: [
          "пасынок",
          "приемный сын",
        ],
        // en: [
        //   "stepson",
        // ],
        fr: [
          "un beau-fils",
        ],
        pl: [
          "pasierb",
        ],
      },
      {
        ru: [
          "падчерица",
          "приемная дочь",
        ],
        // en: [
        //   "stepdaughter",
        // ],
        fr: [
          "une belle-fille",
        ],
        pl: [
          "pasierbica",
        ],
      },
      {
        ru: [
          "сводный брат",
          "брат по одному из родителей",
        ],
        // en: [
        //   "half brother",
        // ],
        fr: [
          "un demi-frère",
        ],
      },
      {
        ru: [
          "сводная сестра",
          "сестра по одному из родителей",
        ],
        // en: [
        //   "half sister",
        // ],
        fr: [
          "une demi soeur",
        ],
      },
      {
        ru: [
          "близкий родственник",
        ],
        // en: [
        //   "close relative",
        // ],
        fr: [
          "(un/une) parent proche",
        ],
      },
      {
        ru: [
          "дальний родственник",
        ],
        // en: [
        //   "distant relative",
        // ],
        fr: [
          "(un/une) parent éloigné"
        ],
      },
      // {
      //   ru: [
      //     "расширенная семья",
      //   ],
      //   en: [
      //     "extended family",
      //   ],
      // },










      /* Дети */
      {
        ru: [
          "ребенок",
        ],
        // en: [
        //   "child",
        //   "kid",
        // ],
        fr: [
          "(un/une) enfant",
        ],
        pl: [
          "dziecko",
        ],
      },
      {
        ru: [
          "дети",
        ],
        // en: [
        //   "children",
        // ],
        fr: [
          "des elefants",
        ],
        pl: [
          "dzieci",
        ],
      },
      {
        ru: [
          "близнецы",
          "двойняшки",
        ],
        // en: [
        //   "twins",
        // ],
        fr: [
          "des jumeaux (о мальчиках)",
          "des jumelles (о девочках)",
        ],
        pl: [
          "bliźnięta",
        ],
      },
      {
        ru: [
          "тройня",
          "тройняшки",
        ],
        // en: [
        //   "triplet babies",
        // ],
        fr: [
          "les trois jumeaux (о мальчиках)",
          "les trois jumelles (о мальчиках)",
        ],
      },
      {
        ru: [
          "сирота",
        ],
        // en: [
        //   "orphan",
        // ],
        fr: [
          "un orphelin (о малчике)",
          "une orpheline (о девочке)",
        ],
      },
      {
        ru: [
          "приемный ребенок",
          "усыновленный ребенок",
        ],
        // en: [
        //   "adopted child",
        // ],
        fr: [
          "un enfant adopté",
        ],
      },
      {
        ru: [
          "единственный ребенок в семье",
        ],
        // en: [
        //   "the only child in the family",
        // ],
        fr: [
          "un enfant unique dans la famille",
        ],
      },










      /* Взаимоотношения */
      // {
      //   ru: [
      //     "семейный сбор",
      //     "семейные посиделки",
      //     "семейное собрание",
      //     "семейная встреча",
      //   ],
      //   en: [
      //     "family gathering",
      //   ],
      // },
      // {
      //   ru: [
      //     "материнский инстинкт",
      //   ],
      //   en: [
      //     "maternal instinct",
      //   ],
      // },
      // {
      //   ru: [
      //     "соперничество между детьми",
      //     "соперничество между братьями",
      //     "соперничество между сестрами",
      //   ],
      //   en: [
      //     "sibling rivalry",
      //   ],
      // },
      // {
      //   ru: [
      //     "поразительное сходство",
      //     "разительное сходство",
      //   ],
      //   en: [
      //     "striking resemblance",
      //   ],
      // },
      // {
      //   ru: [
      //     "соперничество",
      //     "конкуренция",
      //   ],
      //   en: [
      //     "rivalry",
      //   ],
      // },
      // {
      //   ru: [
      //     "сходство",
      //     "похожесть",
      //     "схожесть",
      //   ],
      //   en: [
      //     "resemblance",
      //   ],
      // },
      // {
      //   ru: [
      //     "узы",
      //     "связь",
      //   ],
      //   en: [
      //     "bond",
      //     "connection",
      //     "relation",
      //     "ties",
      //   ],
      // },
      // {
      //   ru: [
      //     "отношения",
      //     "взаимоотношения",
      //   ],
      //   en: [
      //     "relationship",
      //   ],
      // },
      // {
      //   ru: [
      //     "братство",
      //   ],
      //   en: [
      //     "brotherhood",
      //   ],
      // },
      // {
      //   ru: [
      //     "дружба",
      //   ],
      //   en: [
      //     "friendship",
      //   ],
      // },
      {
        ru: [
          "быть похожим на мать",
        ],
        // en: [
        //   "look like mother",
        // ],
        fr: [
          "ressembler à la mère",
        ],
      },
      {
        ru: [
          "быть похожими друг на друга",
          "выглядеть одинаково",
        ],
        // en: [
        //   "look alike",
        // ],
        fr: [
          "se ressembler",
        ],
      },










      /* Семейное положение */
      {
        ru: [
          "семейное положение",
        ],
        // en: [
        //   "marital state",
        // ],
        fr: [
          "une situation de famille",
        ],
      },
      {
        ru: [
          "вдова",
        ],
        // en: [
        //   "widow",
        // ],
        fr: [
          "veuve",
        ],
      },
      {
        ru: [
          "вдовец",
        ],
        // en: [
        //   "widower",
        // ],
        fr: [
          "veuf",
        ],
      },
      {
        ru: [
          "женатый",
          "женат",
          "замужем",
          "замужняя",
        ],
        // en: [
        //   "married",
        // ],
        fr: [
          "marié(e) (женат (замужем))",
          "être marié",
        ],
      },
      {
        ru: [
          "не женат",
          "не замужем",
          "одинокий",
          "одинок",
        ],
        // en: [
        //   "not married",
        //   "single",
        // ],
        fr: [
          "célibataire",
        ],
      },
      {
        ru: [
          "разведен(а)",
          "в разводе",
        ],
        // en: [
        //   "divorced",
        // ],
        fr: [
          "divorcé(e)",
        ],
      },
      









      /* Adjectives */
      // {
      //   ru: [
      //     "сплоченная (~ семья)",
      //     "крепкая (~ семья)",
      //     "дружная (~ семья)",
      //   ],
      //   en: [
      //     "close-knit (~ family)",
      //   ],
      // },
      // {
      //   ru: [
      //     "прочная (~ семья)",
      //     "стабильная (~ семья)",
      //   ],
      //   en: [
      //     "stable (~ family)",
      //   ],
      // },
      // {
      //   ru: [
      //     "материнский",
      //   ],
      //   en: [
      //     "maternal",
      //   ],
      // },
      // {
      //   ru: [
      //     "родительский",
      //   ],
      //   en: [
      //     "parental",
      //   ],
      // },
      {
        ru: [
          "бездетная семья",
        ],
        // en: [
        //   "childless family",
        //   "family with no children",
        // ],
        fr: [
          "une famille sans enfants",
        ],
      },
      {
        ru: [
          "многодетная семья",
          "большая семья",
        ],
        // en: [
        //   "large family",
        // ],
        fr: [
          "une famille nombreuse",
        ],
      },
      // {
      //   ru: [
      //     "неполная семья",
      //     "семья с одним родителем",
      //   ],
      //   en: [
      //     "one-parent family",
      //   ],
      // },
      // {
      //   ru: [
      //     "мать-одиночка",
      //     "одинокая мать",
      //   ],
      //   en: [
      //     "single mother",
      //   ],
      // },










      // Verbs
      // {
      //   ru: [
      //     "принимать (~ ребенка)",
      //     "усыновлять",
      //     "удочерять",
      //   ],
      //   en: [
      //     "adopt",
      //   ],
      // },
      // {
      //   ru: [
      //     "иметь что-то общее",
      //   ],
      //   en: [
      //     "have something in common",
      //   ],
      // },
      // {
      //   ru: [
      //     "наследовать",
      //   ],
      //   en: [
      //     "inherit",
      //   ],
      // },
      // {
      //   ru: [
      //     "ростить",
      //     "выращивать",
      //     "воспитывать",
      //   ],
      //   en: [
      //     "nurture",
      //     "bring up",
      //   ],
      // },
      // {
      //   ru: [
      //     "играть роль",
      //   ],
      //   en: [
      //     "play a role",
      //   ],
      // },
      // {
      //   ru: [
      //     "относиться (к)",
      //     "иметь отношение (к)",
      //   ],
      //   en: [
      //     "relate (to)",
      //   ],
      // },
    ],










    "События в жизни": [
      // Этапы жизни
      {
        ru: [
          "взрослый",
        ],
        en: [
          "adult",
        ],
        fr: [
          "adulte",
        ],
        pl: [
          "dorosły",
        ],
      },
      {
        ru: [
          "младенец",
        ],
        en: [
          "baby",
        ],
        fr: [
          "un bébé",
        ],
        pl: [
          "niemowlę",
        ],
      },
      {
        ru: [
          "подросток",
        ],
        en: [
          "teenager",
          "adolescent",
        ],
        fr: [
          "un adolescente",
        ],
        pl: [
          "nastolatek (м.р.)",
          "nastolatka (ж.р.)",
        ],
      },
      {
        ru: [
          "материнство",
        ],
        en: [
          "motherhood",
        ],
      },
      {
        ru: [
          "взаимодействие",
        ],
        en: [
          "interaction",
        ],
      },
      {
        ru: [
          "ребенок",
        ],
        en: [
          "child",
        ],
        pl: [
          "dziecko",
        ],
      },
      {
        ru: [
          "adolescence",
        ],
        en: [
          "подростковый возраст",
        ],
      },
      {
        ru: [
          "совершеннолетие",
          "взрослая жизнь",
        ],
        en: [
          "adulthood",
        ],
      },
      {
        ru: [
          "детство",
        ],
        en: [
          "childhood",
        ],
      },
      {
        ru: [
          "отцовство",
        ],
        en: [
          "fatherhood",
        ],
      },
      {
        ru: [
          "заводить детей",
        ],
        en: [
          "have children",
        ],
        fr: [
          "avoir des enfants",
        ],
      },
      {
        ru: [
          "рожать ребенка",
        ],
        en: [
          "have a baby",
          "gibe birth to",
        ],
        fr: [
          "avoir un enfant",
          "accoucher un enfant",
        ],
      },
      {
        ru: [
          "родиться",
        ],
        en: [
          "be born",
        ],
        fr: [
          "naître",
        ],
      },










      /* Свадьба */
      {
        ru: [
          "брак",
        ],
        en: [
          "marriage",
        ],
        fr: [
          "un mariage",
        ],
      },
      {
        ru: [
          "свадьба",
        ],
        en: [
          "wedding",
        ],
        fr: [
          "un mariage",
        ],
      },
      {
        ru: [
          "гражданский брак",
        ],
        en: [
          "civil marriage",
        ],
        fr: [
          "un mariage civil",
        ],
      },
      {
        ru: [
          "неравный брак",
        ],
        en: [
          "misalliance",
        ],
        fr: [
          "une mésalliance",
        ],
      },
      {
        ru: [
          "брак по любви",
        ],
        en: [
          "love-match",
        ],
        fr: [
          "un mariage d'amour",
        ],
      },
      {
        ru: [
          "marriage of convenience",
        ],
        en: [
          "брак по расчету",
        ],
        fr: [
          "un mariage de raison",
          "un mariage d'intérêt",
          "un mariage d'argent",
        ],
      },
      {
        ru: [
          "pro forma marriage",
        ],
        en: [
          "фиктивный брак",
        ],
        fr: [
          "un mariage blanc",
        ],
      },

      /* Выражения */
      {
        ru: [
          "жениться на (ком-то)",
          "выходить замуж за (кого-то)",
          "вступать в брак с (кем-то)",
          "создавать семью",
        ],
        en: [
          "get married to (sb)",
          "make a family",
        ],
        fr: [
          "se marier",
          "épouser",
          "créer une famille",
        ],
      },
      {
        ru: [
          "divorce",
        ],
        en: [
          "разводиться",
          "расторгать брак",
        ],
        fr: [
          "divorcer",
        ],
      },
      {
        ru: [
          "справлять свадьбу",
          "отмечать свадьбу",
          "гулять свадьбу",
          "праздновать свадьбу",
        ],
        en: [
          "selebrate a wedding",
        ],
        fr: [
          "célébrer un mariage",
          "célébrer les noces",
        ],
      },
      {
        ru: [
          "быть на свадьбе (на чьей-то)",
        ],
        fr: [
          "être do la noce",
        ],
      },










      // Verbs
      {
        ru: [
          "развивать",
        ],
        en: [
          "develop",
        ],
      },
      {
        ru: [
          "терпеть",
          "выносить",
        ],
        en: [
          "endure",
        ],
      },
    ],










    "Внешность": [
      {
        ru: [
          "какая она?",
        ],
        pl: [
          "jaka ona jest?",
        ],
      },
      {
        ru: [
          "какой он?",
        ],
        pl: [
          "jaki on jest?",
        ],
      },
    ],










    "Личность": [
      {
        ru: [
          "хорошее воспитание",
        ],
        en: [
          "stable upbringing",
        ],
      },
      {
        ru: [
          "активная роль",
        ],
        en: [
          "active role",
        ],
      },
      {
        ru: [
          "воспитание",
        ],
        en: [
          "upbringing",
        ],
      },
      {
        ru: [
          "темперамент",
        ],
        en: [
          "temperament",
        ],
      },
      {
        ru: [
          "природа",
        ],
        en: [
          "nature",
        ],
      },
      {
        ru: [
          "характер",
        ],
        en: [
          "character",
        ],
      },
      {
        ru: [
          "инстинкт",
        ],
        en: [
          "instinct",
        ],
      },
    ],
  },










  "Дом": {
    "Виды": [],
    "Рынок жилья": [],
    "Части дома. Комнаты": [
      {
        ru: [
          "туалет",
        ],
        pl: [
          "toaleta",
        ],
      },
      {
        ru: [
          "кабинет",
        ],
        pl: [
          "gabinet",
        ],
      },
      {
        ru: [
          "дверь",
          "двери",
        ],
        pl: [
          "drzwi",
        ],
      },
      {
        ru: [
          "окно",
        ],
        pl: [
          "okno",
        ],
      },
    ],
    "Гостиная. Мебель": [
      {
        ru: [
          "стул",
        ],
        pl: [
          "krzesło",
        ],
      },
      {
        ru: [
          "телефонная книга",
        ],
        pl: [
          "książka telefoniczna",
        ],
      },
      {
        ru: [
          "лампа",
        ],
        pl: [
          "lampa",
        ],
      },
      {
        ru: [
          "стол",
        ],
        pl: [
          "stół",
        ],
      },
      {
        ru: [
          "снимок",
          "фотография",
        ],
        pl: [
          "zdjęcie",
        ],
      },
    ],
    "Кухня": [],
    "Спальня": [],
    "Прихожая": [],
    "Санузел": [],
    "Вокруг дома": [],
    "Предметы домашнего обихода, Инструменты": [],
    "Работа по дому": [],
  },
  
  
  
  
  
  
  
  
  
  
  "География": {
    "Страны": [
      {
        ru: [
          "Америка",
        ],
        pl: [
          "Ameryka",
        ],
      },
      {
        ru: [
          "Англия",
        ],
        pl: [
          "Anglia",
        ],
      }, {
        ru: [
          "Франция",
        ],
        pl: [
          "Francja",
        ],
      },
      {
        ru: [
          "Испания",
        ],
        pl: [
          "Hiszpania",
        ],
      },
      {
        ru: [
          "Япония",
        ],
        pl: [
          "Japonia",
        ],
      },
      {
        ru: [
          "Германия",
        ],
        pl: [
          "Niemcy",
        ],
      },
      {
        ru: [
          "Польша",
        ],
        pl: [
          "Polska",
        ],
      },
      {
        ru: [
          "Россия",
        ],
        pl: [
          "Rosja",
        ],
      },
      {
        ru: [
          "Италия",
        ],
        pl: [
          "Włochy",
        ],
      },
    ],
    "Национальности": [],
    "Города": [
      {
        ru: [
          "Барселона",
        ],
        pl: [
          "Barcelona",
        ],
      },
      {
        ru: [
          "Берлин",
        ],
        pl: [
          "Berlin",
        ],
      },
      {
        ru: [
          "Бирмингем",
        ],
        pl: [
          "Birmingham",
        ],
      },
      {
        ru: [
          "Эльблонг",
        ],
        pl: [
          "Elbląg",
        ],
      },
      {
        ru: [
          "Гданьск",
        ],
        pl: [
          "Gdańsk",
        ],
      },
      {
        ru: [
          "Катовице",
        ],
        pl: [
          "Katowice",
        ],
      },
      {
        ru: [
          "Колобжег",
        ],
        pl: [
          "Kołobrzeg",
        ],
      },
      {
        ru: [
          "Краков",
        ],
        pl: [
          "Kraków",
        ],
      },
      {
        ru: [
          "Лодзь",
        ],
        pl: [
          "Łódź",
        ],
      },
      {
        ru: [
          "Лондон",
        ],
        pl: [
          "Londyn",
        ],
      },
      {
        ru: [
          "Москва",
        ],
        pl: [
          "Moskwa",
        ],
      },
      {
        ru: [
          "Нью-Йорк",
        ],
        pl: [
          "Nowy Jork",
        ],
      },
      {
        ru: [
          "Париж",
        ],
        pl: [
          "Paryż",
        ],
      },
      {
        ru: [
          "Познань",
        ],
        pl: [
          "Poznań",
        ],
      },
      {
        ru: [
          "Жешув",
        ],
        pl: [
          "Rzeszów",
        ],
      },
      {
        ru: [
          "Рим",
        ],
        pl: [
          "Rzym",
        ],
      },
      {
        ru: [
          "Сувалки",
        ],
        pl: [
          "Suwałki",
        ],
      },
      {
        ru: [
          "Щецин",
        ],
        pl: [
          "Szczecin",
        ],
      },
      {
        ru: [
          "Токио",
        ],
        pl: [
          "Tokio",
        ],
      },
      {
        ru: [
          "Варшава",
        ],
        en: [
          "Warszawa",
        ],
      },
      {
        ru: [
          "Вроцлав",
        ],
        pl: [
          "Wrocław",
        ],
      },
      {
        ru: [
          "Закопане",
        ],
        pl: [
          "Zakopane",
        ],
      },
      {
        ru: [
          "Замостье",
          "Замосць",
        ],
        pl: [
          "Zamość",
        ],
      },
      {
        ru: [
          "Зелена Гура",
        ],
        pl: [
          "Zielona Góra",
        ],
      },
      {
        ru: [
          "Ченстохова",
        ],
        pl: [
          "Częstochowa",
        ],
      },
    ],
    "Реки": [
      {
        ru: [
          "Буг",
        ],
        pl: [
          "Bug",
        ],
      },
      {
        ru: [
          "Одра",
          "Одер",
        ],
        pl: [
          "Odra",
        ],
      },
      {
        ru: [
          "Ватра",
        ],
        pl: [
          "Watra",
        ],
      },
      {
        ru: [
          "Висла",
        ],
        pl: [
          "Wisła",
        ],
      },
    ],
  },
  
  
  
  
  
  
  
  
  
  
  "Медицина и здоровье": {
    "В больнице": [],





    
    
    
    
    
    "Недомогания и болезни": [],
    
    
    
    
    
    
    
    
    
    
    "Части тело и органы человека": [],
    





    
    
    
    
    "Забота о здоровье": [
      /* Уход за телом */
      {
        ru: [
          "принимать душ",
        ],
        en: [
          "take a shower",
          "have a shower",
        ],
      },
    ],
    
    





    
    
    
    "В аптеке": [],
  },
  
  
  
  
  
  
  
  
  
  
  "Магазины и услуги": {
    "Магазин": [],










    "Деньги и платежи": [
      {
        ru: [
          "деньги",
        ],
        en: [
          "money",
        ],
      },
    ],
    
    
    
    
    
    
    
    
    
    
    "Магазины и отделы": {
      "Электротовары": [
        {
          ru: [
            "телефон",
          ],
          pl: [
            "telefon",
          ],
        },
        {
          ru: [
            "домашний телефон",
          ],
          pl: [
            "telefon domowy",
          ],
        },
        {
          ru: [
            "сотовый телефон",
            "мобильный телефон",
          ],
          pl: [
            "telefon komórkowy",
          ],
        },
        {
          ru: [
            "мобилка",
            "мобильный",
          ],
          pl: [
            "komórka",
          ],
        },
        {
          ru: [
            "компьютер",
          ],
          en: [
            "computer",
          ],
          pl: [
            "komputer",
          ],
        },
      ],
    },
    
    
    
    
    
    
    
    
    
    
    "Одежда": [],
    
    
    
    
    
    
    
    
    
    
    "Аксессуары": [
      {
        ru: [
          "очки",
        ],
        pl: [
          "okulary",
        ],
      },
    ],
    
    
    
    
    
    
    
    
    
    
    "Обувь": [],
    
    
    
    
    
    
    
    
    
    
    "Стиль": [],
  },
  
  
  
  
  
  
  
  
  
  
  "Еда": {
    "Кухня": [],
    "Овощи": [],
    "Напитки": [
      {
        ru: [
          "чай",
        ],
        pl: [
          "чай",
        ],
      },
      {
        ru: [
          "кофе",
        ],
        pl: [
          "kawa",
        ],
      },
      {
        ru: [
          "пиво",
        ],
        pl: [
          "piwo",
        ],
      },
      {
        ru: [
          "вино",
        ],
        pl: [
          "wino",
        ],
      },
    ],
    "Кобласно-мясные изделия": [],
    "Хлебобулочные изделия": [],
    "Фрукты, ягоды, орехи, грибы": [],
    "В бакалее": [],
    "Молокопродукты, яйца и сладости": [],
    "Рыба и морепродукты": [],
    "Готовим еду. Продукты": [],
    "Серверуем стол": [],
    "В ресторане": [],
  },
  
  
  
  
  
  
  
  
  
  
  "Мир спорта": {
    "Виды спорта": [],










    "Игры": [],
    
    
    
    
    
    
    
    
    
    
    "Спорт": [
      /* Спортсмены */
      {
        ru: [
          "футболист",
        ],
        en: [
          "football player",
          "footballer",
        ],
      },
    ],
    
    
    
    
    
    
    
    
    
    
    "Спортивная деятельность": [],
  },
  
  
  
  
  
  
  
  
  
  
  "Природа": {
    "Природа": [
      {
        ru: [
          "природа",
        ],
        en: [
          "nature",
        ],
      },
    ],
    "Небо": [
      {
        ru: [
          "солнце",
        ],
        pl: [
          "słońce",
        ],
      },
    ],
    "Планета Земля": [],
    "Водоемы": [
      {
        ru: [
          "река",
        ],
        pl: [
          "rzeka",
        ],
      }
    ],
    "Природные ресурсы": [],
    "Защита окружающей среды": [],
    "Человеческая деятельность": [],
    "Растительность, растения": [],
    "Животные": [
      {
        ru: [
          "кот",
        ],
        pl: [
          "kot",
        ],
      },
      {
        ru: [
          "собака",
          "пес",
        ],
        pl: [
          "pies",
        ],
      },
    ],
    "Птицы": [],
    "Ареал обитания. Характеристики": [],
  },
  
  
  
  
  
  
  
  
  
  
  "Путешествия": {
    "Готовимся к поездке": [],
    "Транспорт": [],
    "На вокзале": [],
    "Путешествие поездом": [],
    "Путешествие на самолете": [],
    "Морское путешествие": [],
    "Автомобили": [],
  },
  
  
  
  
  
  
  
  
  
  
  "Профессии, Бизнес": {
    "Работа. Бизнес": [
      {
        ru: [
          "фирма",
        ],
        pl: [
          "firma",
        ],
      },
      {
        ru: [
          "проект",
        ],
        pl: [
          "projekt",
        ],
      },
      {
        ru: [
          "сервис",
        ],
        pl: [
          "serwis",
        ],
      },
      {
        ru: [
          "усердно работать",
          "тяжело работать",
        ],
        en: [
          "work hard",
        ],
      },
    ],










    "Профессии": [
      {
        ru: [
          "актер",
        ],
        en: [
          "actor",
        ],
        pl: [
          "aktor",
        ],
      },
      {
        ru: [
          "актриса",
        ],
        en: [
          "actress",
        ],
        pl: [
          "aktorka",
        ],
      },
      {
        ru: [
          "учитель",
        ],
        en: [
          "teacher",
        ],
        pl: [
          "nauczyciel",
        ],
      },
      {
        ru: [
          "художник",
        ],
        en: [
          "artist",
        ],
        pl: [
          "artysta",
        ],
      },
      {
        ru: [
          "художница",
        ],
        pl: [
          "artystka",
        ],
      },
      {
        ru: [
          "политик",
        ],
        pl: [
          "polityk",
        ],
      },
      {
        ru: [
          "поэт",
        ],
        pl: [
          "poeta",
        ],
      },
      {
        ru: [
          "поэтесса",
        ],
        pl: [
          "poetka",
        ],
      },
    ],
  },
  
  
  
  
  
  
  
  
  
  
  "Технологии и коммуникации": {
    "Наука и технология": [
      {
        ru: [
          "профессор",
        ],
        pl: [
          "profesor",
        ],
      },
    ],










    "Средства коммуникации": [
      {
        ru: [
          "газета",
        ],
        pl: [
          "gazeta",
        ],
      },
      {
        ru: [
          "журнал",
        ],
        pl: [
          "magazyn",
        ],
      },
      {
        ru: [
          "радио",
        ],
        pl: [
          "radio",
        ],
      },
      {
        ru: [
          "телефон",
        ],
        pl: [
          "telefon",
        ],
      },
      {
        ru: [
          "письмо",
        ],
        en: [
          "letter",
        ],
      },
    ],
  },
  
  
  
  
  
  
  
  
  
  
  "Учеба": {
    "Образование": [
      {
        ru: [
          "класс",
        ],
        en: [
          "class",
        ],
      },
      {
        ru: [
          "философия",
        ],
        pl: [
          "filozofia",
        ],
      },
      {
        ru: [
          "упражнение",
        ],
        pl: [
          "ćwiczenie",
        ],
      },
      {
        ru: [
          "урок",
          "занятие",
        ],
        en: [
          "class",
        ],
        pl: [
          "lekcja",
        ],
      },
      {
        ru: [
          "учебник",
        ],
        pl: [
          "podręcznik",
        ],
      },
      {
        ru: [
          "тетрадь",
        ],
        pl: [
          "zeszyt",
        ],
      },
      {
        ru: [
          "студент",
        ],
        pl: [
          "student",
        ],
      },
      {
        ru: [
          "студентка",
        ],
        pl: [
          "studentka",
        ],
      },
      {
        ru: [
          "учитель",
        ],
        en: [
          "teacher",
        ],
        pl: [
          "nauczyciel",
        ],
      },
      {
        ru: [
          "звукозапись",
          "запись",
        ],
        pl: [
          "nagranie",
        ],
      },
      {
        ru: [
          "профессор",
        ],
        pl: [
          "profesor",
        ],
      },
      {
        ru: [
          "гимназия",
        ],
        pl: [
          "gimnazjum",
        ],
      },
      {
        ru: [
          "школа",
        ],
        pl: [
          "szkoła",
        ],
      },
      {
        ru: [
          "шариковая ручка",
          "ручка",
        ],
        pl: [
          "długopis",
        ],
      },
      {
        ru: [
          "текст",
        ],
        pl: [
          "tekst",
        ],
      },
      {
        ru: [
          "группа",
        ],
        pl: [
          "grupa",
        ],
      },
      {
        ru: [
          "усердно учиться",
          "прележно учиться",
        ],
        en: [
          "study hard",
        ],
      },
      {
        ru: [
          "книга",
        ],
        en: [
          "book",
        ],
        pl: [
          "książka",
        ],
      },
    ],










    "Языки": [
      {
        ru: [
          "буква",
        ],
        en: [
          "letter",
        ],
      },
      {
        ru: [
          "акцент",
          "ударение",
        ],
        pl: [
          "akcent",
        ],
      },
      {
        ru: [
          "грамматика",
        ],
        pl: [
          "gramatyka",
        ],
      },
      {
        ru: [
          "орфография",
          "правописание",
        ],
        pl: [
          "ortografia",
        ],
      },
      {
        ru: [
          "предлог",
        ],
        pl: [
          "przyimek",
        ],
      },
      {
        ru: [
          "прилагательное",
        ],
        pl: [
          "przymiotnik",
        ],
      },
      {
        ru: [
          "падеж",
          "случай",
        ],
        pl: [
          "przypadek",
        ],
      },
      {
        ru: [
          "наречие",
        ],
        pl: [
          "przysłówek",
        ],
      },
      {
        ru: [
          "мужской род",
        ],
        pl: [
          "rodzaj męski",
        ],
      },
      {
        ru: [
          "средний род",
        ],
        pl: [
          "rodzaj nijaki",
        ],
      },
      {
        ru: [
          "женский род",
        ],
        pl: [
          "rodzaj żeński",
        ],
      },
      {
        ru: [
          "числительное",
        ],
        pl: [
          "liczebnik",
        ],
      },
      {
        ru: [
          "существительное",
        ],
        pl: [
          "rzeczownik",
        ],
      },
      {
        ru: [
          "гласный",
        ],
        pl: [
          "samogłoska",
        ],
      },
      {
        ru: [
          "согласный",
        ],
        pl: [
          "spółgłoska",
        ],
      },
      {
        ru: [
          "лексика",
          "словарный запас",
        ],
        pl: [
          "słownictwo",
        ],
      },
      {
        ru: [
          "слово",
        ],
        pl: [
          "słowo",
        ],
      },
      {
        ru: [
          "язык",
        ],
        pl: [
          "język",
        ],
      },
      {
        ru: [
          "неофициальный язык",
        ],
        pl: [
          "język nieoficjalny",
        ],
      },
      {
        ru: [
          "официальный язык",
        ],
        pl: [
          "język oficjalny",
        ],
      },
      {
        ru: [
          "универсальный язык",
        ],
        pl: [
          "język uniwersalny",
        ],
      },
      {
        ru: [
          "множественное число",
        ],
        pl: [
          "liczba mnoga",
        ],
      },
      {
        ru: [
          "единственное число",
        ],
        pl: [
          "liczba pojedyncza",
        ],
      },
      {
        ru: [
          "инфинитив",
        ],
        pl: [
          "bezokolicznik",
        ],
      },
      {
        ru: [
          "глагол",
        ],
        pl: [
          "czasownik",
        ],
      },
      {
        ru: [
          "именительный падеж (кто? что?)",
        ],
        pl: [
          "mianownik",
        ],
      },
      {
        ru: [
          "родительный падеж (кому? чему?)",
        ],
        pl: [
          "dopełniacz",
        ],
      },
      {
        ru: [
          "дательный падеж (кого? чего?)",
        ],
        pl: [
          "celownik",
        ],
      },
      {
        ru: [
          "винительный падеж (кого? что?)",
        ],
        pl: [
          "biernik",
        ],
      },
      {
        ru: [
          "предложный падеж (о ком? о чем?)",
        ],
        pl: [
          "miejscownik",
        ],
      },
      {
        ru: [
          "творительный падеж (кем? чем?)",
        ],
        pl: [
          "narzędnik",
        ],
      },
      {
        ru: [
          "звательный падеж",
        ],
        pl: [
          "wołacz",
        ],
      },
      {
        ru: [
          "произношение",
        ],
        pl: [
          "wymowa",
        ],
      },
      {
        ru: [
          "местоимение",
        ],
        pl: [
          "zaimek",
        ],
      },
      {
        ru: [
          "личное местоимение",
        ],
        pl: [
          "zaimek osobowy",
        ],
      },
      {
        ru: [
          "прошедшее время",
        ],
        pl: [
          "czas przeszły",
        ],
      },
      {
        ru: [
          "будущее время",
        ],
        pl: [
          "czas przyszły",
        ],
      },
      {
        ru: [
          "настоящее время",
        ],
        pl: [
          "czas teraźniejszy",
        ],
      },
      {
        ru: [
          "предложение (часть речи)",
        ],
        pl: [
          "zdanie",
        ],
      },
    ],
    
    
    
    
    
    
    
    
    
    
    "Числительные": [
      {
        ru: [
          "ноль",
          "0",
        ],
        pl: [
          "zero",
        ],
      },
      {
        ru: [
          "один",
          "1",
        ],
        pl: [
          "jeden",
        ],
      },
      {
        ru: [
          "два",
          "2",
        ],
        pl: [
          "dwa",
        ],
      },
      {
        ru: [
          "три",
          "3",
        ],
        pl: [
          "trzy",
        ],
      },
      {
        ru: [
          "четыре",
          "4",
        ],
        pl: [
          "cztery",
        ],
      },
      {
        ru: [
          "пять",
          "5",
        ],
        pl: [
          "pięć",
        ],
      },
      {
        ru: [
          "шесть",
          "6",
        ],
        pl: [
          "sześć",
        ],
      },
      {
        ru: [
          "семь",
          "7",
        ],
        pl: [
          "siedem",
        ],
      },
      {
        ru: [
          "восемь",
          "8",
        ],
        pl: [
          "osiem",
        ],
      },
      {
        ru: [
          "девять",
          "9",
        ],
        pl: [
          "dziewięć",
        ],
      },
      {
        ru: [
          "десять",
          "10",
        ],
        pl: [
          "dziesięć",
        ],
      },
      {
        ru: [
          "одиннадцать",
          "11",
        ],
        pl: [
          "jedenaście",
        ],
      },
      {
        ru: [
          "двенадцать",
          "12",
        ],
        pl: [
          "dwanaście",
        ],
      },
      {
        ru: [
          "тринадцать",
          "13",
        ],
        pl: [
          "trzynaście",
        ],
      },
      {
        ru: [
          "четырнадцать",
          "14",
        ],
        pl: [
          "czternaście",
        ],
      },
      {
        ru: [
          "пятнадцать",
          "15",
        ],
        pl: [
          "piętnaście",
        ],
      },
      {
        ru: [
          "шестнадцать",
          "16",
        ],
        pl: [
          "szesnaście",
        ],
      },
      {
        ru: [
          "семнадцать",
          "17",
        ],
        pl: [
          "siedemnaście",
        ],
      },
      {
        ru: [
          "восемнадцать",
          "18",
        ],
        pl: [
          "osiemnaście",
        ],
      },
      {
        ru: [
          "девятнадцать",
          "19",
        ],
        pl: [
          "dziewiętnaście",
        ],
      },
      {
        ru: [
          "двадцать",
          "20",
        ],
        pl: [
          "dwadzieścia",
        ],
      },
      {
        ru: [
          "двадцать один",
          "21",
        ],
        pl: [
          "dwadzieścia jeden",
        ],
      },
      {
        ru: [
          "двадцать два",
          "22",
        ],
        pl: [
          "dwadzieścia dwa",
        ],
      },
      {
        ru: [
          "двадцать три",
          "23",
        ],
        pl: [
          "dwadzieścia trzy",
        ],
      },
    ],
    
    
    
    
    
    
    
    
    
    
    "Математика": [
      {
        ru: [
          "2+3 будет 5",
        ],
        pl: [
          "2+3 jest 5",
          "2+3 to jest 5",
        ],
      },
      {
        ru: [
          "сколько будет 2+3?",
        ],
        pl: [
          "Ile jest 2+3?",
        ],
      },
      {
        ru: [
          "минус",
        ],
        pl: [
          "minus",
        ],
      },
      {
        ru: [
          "плюс",
        ],
        pl: [
          "plus",
        ],
      },
      {
        ru: [
          "список",
        ],
        pl: [
          "spis",
        ],
      },
    ],
    
    
    
    
    
    
    
    
    
    
    "Работа с данными": [
      /* Представление данных */
      {
        ru: [
          "диаграмма",
          "график",
        ],
        en: [
          "diagram",
          "graph",
          "chart",
        ],
      },
      {
        ru: [
          "блок-схема (описывает как что-то делается)",
          "технологическая схема",
        ],
        en: [
          "flow chart",
        ],
      },
      {
        ru: [
          "линейный график (изменения на протяжение какого-то времени)",
        ],
        en: [
          "line chart",
          "line graph",
        ],
      },
      {
        ru: [
          "таблица",
        ],
        en: [
          "table",
        ],
      },
      {
        ru: [
          "круговая диаграмма (процентное соотношение от 100%)",
          "секторная диаграмма",
        ],
        en: [
          "pie chart",
        ],
      },
      {
        ru: [
          "гистограмма (диаграмма в виде столбцов)",
        ],
        en: [
          "column graph",
          "bar graph",
        ],
      },










      /* Изменение данных / Увеличение */
      {
        ru: [
          "подниматься",
          "увеличиваться",
          "расти",
          "возрастать",
        ],
        en: [
          "go up",
          "increase",
          "rise",
        ],
      },
      {
        ru: [
          "достигать пика в (значение)",
        ],
        en: [
          "peak at",
        ],
      },
      {
        ru: [
          "достигать пика в (чем-то ~ отрасль)",
        ],
        en: [
          "reach a peak of",
        ],
      },
      {
        ru: [
          "значительный рост",
          "значительное увеличение",
        ],
        en: [
          "significant rise",
          "significant increase",
        ],
      },
      {
        ru: [
          "значительно вырос",
          "значительно увеличился",
        ],
        en: [
          "rose significantly",
          "increased significantly",
        ],
      },










      /* Изменение данных / Уменьшение */
      {
        ru: [
          "уменьшаться",
          "сокращаться",
          "снижаться",
          "спускаться",
          "опускаться",
        ],
        en: [
          "decline",
          "decrease",
          "go down",
        ],
      },
      {
        ru: [
          "падать",
        ],
        en: [
          "drop",
          "fall",
        ],
      },










      /* Изменение данных / Колебание */
      {
        ru: [
          "колебаться",
        ],
        en: [
          "fluctuate",
        ],
      },










      /* Изменение данных / Постоянство */
      {
        ru: [
          "оставаться постоянным",
          "оставаться устойчивым",
        ],
        en: [
          "remain constant",
          "remain steady",
        ],
      },










      /* Характеристика изменений */
      {
        ru: [
          "тенденция к понижению",
          "тенденция к снижению",
        ],
        en: [
          "downward trend",
        ],
      },
      {
        ru: [
          "тенденция к повышению",
          "тенденция роста",
        ],
        en: [
          "upward trend",
        ],
      },
      {
        ru: [
          "резко",
          "драматично",
        ],
        en: [
          "sharply",
          "dramatically",
        ],
      },
      {
        ru: [
          "понемногу",
          "постепенно",
        ],
        en: [
          "gradually",
        ],
      },
      {
        ru: [
          "слегка",
          "немного",
        ],
        en: [
          "slightly",
        ],
      },
      {
        ru: [
          "неуклонно",
          "постоянно",
        ],
        en: [
          "steadily",
        ],
      },










      /* Описание значений */
      {
        ru: [
          "стоить $5 за ... (единицу)",
        ],
        en: [
          "stand at $5 per ...",
        ],
      },
      {
        ru: [
          "состовлять 10% от общего ...",
        ],
        en: [
          "account for 10% total ...",
        ],
      },
      {
        ru: [
          "с 1990 по 1995",
          "между 1990 и 1995",
        ],
        en: [
          "from 1990 to 1995",
          "between 1990 and 1995",
        ],
      },
      {
        ru: [
          "в период с 1990 по 1995",
          "на протяжении периода с 1990 по 1995",
        ],
        en: [
          "during the period from 1990 to 1995",
          "throughout the period from 1990 to 1995",
        ],
      },
      {
        ru: [
          "в течение периода в 5 лет",
          "на протяжении периода в 5 лет",
          "на протяжении 5 лет",
          "в течение 5 лет",
          "в течение пятилетнего периода",
          "на протяжении пятилетнего периода",
          "за пятилетний период",
        ],
        en: [
          "during a period of 5 years",
          "for 5 years",
          "during 5 years",
          "over the five-year period",
        ],
      },
      {
        ru: [
          "в 1990 году",
        ],
        en: [
          "in 1990",
        ],
      },
      {
        ru: [
          "к 1995 году",
        ],
        en: [
          "by 1995",
        ],
      },
    ],
  },
  
  
  
  
  
  
  
  
  
  
  "Город. Деревня": {
    "Город": [
      {
        ru: [
          "город",
        ],
        pl: [
          "miasto",
        ],
      },
      {
        ru: [
          "микрорайон",
          "жилой комплекс",
          "ЖК",
        ],
        pl: [
          "osiedle",
        ],
      },
      {
        ru: [
          "бензозаправка",
          "заправка",
        ],
        pl: [
          "stacja benzynowa",
        ],
      },
      {
        ru: [
          "площадь",
        ],
        pl: [
          "plac",
        ],
      },
      {
        ru: [
          "улица",
        ],
        pl: [
          "ulica",
        ],
      },
      {
        ru: [
          "аллея",
          "проспект",
        ],
        pl: [
          "aleja",
        ],
      },
      {
        ru: [
          "дом",
        ],
        pl: [
          "dom",
        ],
      },
      {
        ru: [
          "гостиница",
          "отель",
        ],
        pl: [
          "hotel",
        ],
      },
      {
        ru: [
          "архитектура",
        ],
        pl: [
          "architektura",
        ],
      },
      {
        ru: [
          "банк",
        ],
        pl: [
          "bank",
        ],
      },
      {
        ru: [
          "офис",
          "бюро",
        ],
        pl: [
          "biuro",
        ],
      },
      {
        ru: [
          "центр",
        ],
        pl: [
          "centrum",
        ],
      },
      {
        ru: [
          "в центре",
        ],
        pl: [
          "jest w centrum",
        ],
      },
      {
        ru: [
          "гимназия",
        ],
        pl: [
          "gimnazjum",
        ],
      },
      {
        ru: [
          "музей",
        ],
        pl: [
          "muzeum",
        ],
      },
      {
        ru: [
          "парк",
        ],
        pl: [
          "park",
        ],
      },
      {
        ru: [
          "ресторан",
        ],
        pl: [
          "restauracja",
        ],
      },
      {
        ru: [
          "школа",
        ],
        pl: [
          "szkoła",
        ],
      },
      {
        ru: [
          "музыкальная школа",
        ],
        pl: [
          "szkoła muzyczna",
        ],
      },
    ],
    "Дорога": [
      {
        ru: [
          "автомобиль",
          "авто",
          "машина",
        ],
        pl: [
          "samochód",
          "auto",
        ],
      },
      {
        ru: [
          "автобус",
        ],
        pl: [
          "autobus",
        ],
      },
    ],
    "В деревне, за городом": [
      {
        ru: [
          "дом",
        ],
        pl: [
          "dom",
        ],
      },
    ],
  },
  
  
  
  
  
  
  
  
  
  
  "Календарь. Время. Климат": {
    "Погода. Климат": [],
    "Время": [],
    "Погода": [],
    "Календарь": [
      {
        ru: [
          "день",
        ],
        pl: [
          "dzień",
        ],
      },
      {
        ru: [
          "вечер",
        ],
        pl: [
          "wieczór",
        ],
      },
    ],
  },
  
  
  
  
  
  
  
  
  
  
  "Досуг. Хобби. Искусство": {
    "Досуг. Хобби": [
      {
        ru: [
          "фотография",
        ],
        pl: [
          "fotografia",
        ],
      },
    ],
    "Искусство": [
      {
        ru: [
          "кино",
        ],
        pl: [
          "kino",
        ],
      },
      {
        ru: [
          "литература",
        ],
        pl: [
          "literatura",
        ],
      },
      {
        ru: [
          "музыка",
        ],
        pl: [
          "muzyka",
        ],
      },
      {
        ru: [
          "фонд культуры",
        ],
        pl: [
          "fundacja kultury",
        ],
      }
    ],
    "Театр": [
      {
        ru: [
          "актер",
        ],
        en: [
          "actor",
        ],
        pl: [
          "aktor",
        ],
      },
      {
        ru: [
          "актриса",
        ],
        en: [
          "actress",
        ],
        pl: [
          "aktorka",
        ],
      },

      // Verbs
      {
        ru: [
          "играть роль",
        ],
        en: [
          "play a role",
        ],
      },
    ],
    "Кино": [
      {
        ru: [
          "кино",
        ],
        en: [
          "cinema",
        ],
        pl: [
          "kino",
        ],
      },
      {
        ru: [
          "актер",
        ],
        en: [
          "actor",
        ],
        pl: [
          "aktor",
        ],
      },
      {
        ru: [
          "актриса",
        ],
        en: [
          "actress",
        ],
        pl: [
          "aktorka",
        ],
      },
      {
        ru: [
          "фильм",
        ],
        en: [
          "film",
          "movie",
        ],
        pl: [
          "film",
        ],
      },

      // Verbs
      {
        ru: [
          "играть роль",
        ],
        en: [
          "play a role",
        ],
      },
    ],
    "Живопись и музыка": [
      {
        ru: [
          "художник",
        ],
        en: [
          "artist",
        ],
        pl: [
          "artysta",
        ],
      },
      {
        ru: [
          "художница",
        ],
        pl: [
          "artystka",
        ],
      },
    ],
    "Литература": [
      {
        ru: [
          "поэт",
        ],
        pl: [
          "poeta",
        ],
      },
      {
        ru: [
          "поэтесса",
        ],
        pl: [
          "poetka",
        ],
      },
      {
        ru: [
          "книга",
        ],
        en: [
          "книга",
        ],
        pl: [
          "książka",
        ],
      },
      {
        ru: [
          "персонаж",
        ],
        en: [
          "character",
        ],
      },
    ],
    "Праздники": [
      {
        ru: [
          "традиция",
        ],
        pl: [
          "tradycja",
        ],
      },
    ],
  },










  "Глаголы (Базовые глаголы)": [
    {
      ru: [
        "концентрироваться",
      ],
      en: [
        "concentrate",
      ],
    },
    {
      ru: [
        "концентрироваться",
      ],
      en: [
        "try",
      ],
    },
    {
      ru: [
        "устанавливать",
        "учреждать",
      ],
      en: [
        "establish",
      ],
    },
    {
      ru: [
        "сломать",
      ],
      en: [
        "break down",
      ],
    },
    {
      ru: [
        "спорить",
        "ругаться",
      ],
      en: [
        "argue",
      ],
    },
    {
      ru: [
        "соглашаться",
      ],
      en: [
        "agree",
      ],
    },
    {
      ru: [
        "жаловаться",
      ],
      en: [
        "complain",
      ],
    },
    {
      ru: [
        "зарабатывать (~ деньги)",
      ],
      en: [
        "earn",
      ],
    },
    {
      ru: [
        "улучшать",
      ],
      en: [
        "improve",
      ],
    },
    {
      ru: [
        "тренироваться",
      ],
      en: [
        "train",
      ],
    },
    {
      ru: [
        "относиться (к)",
      ],
      en: [
        "relate (to)",
      ],
    },
    // {
    //   ru: [
    //     "быть",
    //     "являться",
    //   ],
    //   en: ["be"],
    //   pl: ["być"],
    // },
    // {
    //   ru: [
    //     "становиться",
    //     "стать",
    //   ],
    //   en: ["become"],
    // },
    // {
    //   ru: ["начинать"],
    //   en: [
    //     "begin",
    //     "start",
    //   ],
    // },
    // {
    //   ru: ["заканчивать"],
    //   en: [
    //     "finish",
    //     "end",
    //   ],
    // },
    // {
    //   ru: [
    //     "приносить",
    //     "привозить",
    //     "приводить",
    //   ],
    //   en: ["bring"],
    // },
    // {
    //   ru: [
    //     "бить",
    //     "колотить",
    //   ],
    //   en: ["beat"],
    // },
    // {
    //   ru: [
    //     "держать пари",
    //     "спорить",
    //     "делать ставку",
    //     "ставить ставку",
    //   ],
    //   en: ["bet"],
    // },
    // {
    //   ru: [
    //     "кусать",
    //   ],
    //   en: [
    //     "bite",
    //   ],
    // },
    // {
    //   ru: [
    //     "дуть",
    //     "выдыхать",
    //   ],
    //   en: [
    //     "blow",
    //   ],
    // },
    // {
    //   ru: [
    //     "ломать",
    //     "разбивать",
    //   ],
    //   en: [
    //     "break",
    //   ],
    // },
    // {
    //   ru: [
    //     "резать",
    //     "разрезать",
    //   ],
    //   en: [
    //     "cut",
    //   ],
    // },
    // {
    //   ru: [
    //     "строить",
    //     "сооружать",
    //   ],
    //   en: ["build"],
    // },
    // {
    //   ru: [
    //     "гореть",
    //     "жечь",
    //   ],
    //   en: [
    //     "burn",
    //   ],
    // },
    // {
    //   ru: [
    //     "взрываться",
    //     "лопаться",
    //     "взорваться",
    //   ],
    //   en: [
    //     "burst",
    //   ],
    // },
    // {
    //   ru: [
    //     "покупать",
    //   ],
    //   en: [
    //     "buy",
    //   ],
    // },
    // {
    //   ru: [
    //     "мочь",
    //     "уметь",
    //   ],
    //   en: [
    //     "can",
    //   ],
    //   pl: [
    //     "umieć",
    //   ],
    // },
    // {
    //   ru: [
    //     "ловить",
    //     "поймать",
    //   ],
    //   en: [
    //     "catch",
    //   ],
    // },
    // {
    //   ru: [
    //     "выбирать",
    //     "избирать",
    //   ],
    //   en: [
    //     "choose",
    //     "select"
    //   ],
    // },
    // {
    //   ru: [
    //     "приходить",
    //     "приезжать",
    //   ],
    //   en: [
    //     "come",
    //   ],
    // },
    // {
    //   ru: [
    //     "стоить",
    //     "обходиться",
    //   ],
    //   en: [
    //     "cost",
    //   ],
    // },
    // {
    //   ru: [
    //     "кормить",
    //   ],
    //   en: [
    //     "feed",
    //   ],
    // },
    // {
    //   ru: [
    //     "падать",
    //   ],
    //   en: [
    //     "fall",
    //   ],
    // },
    {
      ru: [
        "кушать",
        "есть",
      ],
      // en: [
      //   "eat",
      // ],
      pl: [
        "jeść",
      ],
    },
    {
      ru: [
        "подводить итог",
      ],
      en: [
        "summarise",
      ],
    },
    // {
    //   ru: [
    //     "водить машину",
    //     "ездить",
    //   ],
    //   en: [
    //     "drive",
    //   ],
    // },
    // {
    //   ru: [
    //     "пить",
    //   ],
    //   en: [
    //     "drink",
    //   ],
    // },
    // {
    //   ru: [
    //     "мечтать",
    //     "видеть сны",
    //     "сниться",
    //   ],
    //   en: [
    //     "dream",
    //   ],
    // },
    // {
    //   ru: [
    //     "рисовать",
    //     "чертить",
    //   ],
    //   en: [
    //     "draw",
    //   ],
    // },
    {
      ru: [
        "делать",
        "выполнять",
      ],
      en: [
        // "do",
        "fulfill",
      ],
    },
    {
      ru: [
        "брать в долг",
        "занимать",
        "брать взаймы",
      ],
      en: [
        "borrow",
      ],
    },
    // {
    //   ru: [
    //     "копать",
    //     "рыть",
    //   ],
    //   en: [
    //     "dig",
    //   ]
    // },
    // {
    //   ru: [
    //     "иметь дело",
    //   ],
    //   en: [
    //     "deal",
    //   ],
    // },
    {
      ru: [
        "идти",
        "ехать",
      ],
      // en: [
      //   "go",
      // ],
      pl: [
        "jechać",
      ],
    },
    // {
    //   ru: [
    //     "давать",
    //   ],
    //   en: [
    //     "give",
    //   ],
    // },
    // {
    //   ru: [
    //     "получать",
    //   ],
    //   en: [
    //     "get",
    //   ],
    // },
    // {
    //   ru: [
    //     "замораживать",
    //     "морозить",
    //   ],
    //   en: [
    //     "freeze",
    //   ],
    // },
    // {
    //   ru: [
    //     "прощать",
    //     "простить",
    //   ],
    //   en: [
    //     "forgive",
    //   ],
    // },
    // {
    //   ru: [
    //     "забывать",
    //   ],
    //   en: [
    //     "forget",
    //   ],
    // },
    // {
    //   ru: [
    //     "летать",
    //   ],
    //   en: [
    //     "fly",
    //   ],
    // },
    // {
    //   ru: [
    //     "находить",
    //   ],
    //   en: [
    //     "find",
    //   ],
    // },
    // {
    //   ru: [
    //     "драться",
    //   ],
    //   en: [
    //     "fight",
    //   ],
    // },
    // {
    //   ru: [
    //     "чувствовать (себя)",
    //     "ощущать",
    //   ],
    //   en: [
    //     "feel",
    //   ],
    // },
    // {
    //   ru: [
    //     "стоять на коленях",
    //     "становиться на колени",
    //   ],
    //   en: [
    //     "kneel",
    //   ],
    // },
    // {
    //   ru: [
    //     "хранить",
    //     "сохранять",
    //     "поддерживать",
    //   ],
    //   en: [
    //     "keep",
    //   ],
    // },
    // {
    //   ru: [
    //     "причинять боль",
    //     "ранить",
    //     "ушибить",
    //   ],
    //   en: [
    //     "hurt",
    //   ],
    // },
    // {
    //   ru: [
    //     "держать",
    //     "удерживать",
    //   ],
    //   en: [
    //     "hold",
    //   ],
    // },
    // {
    //   ru: [
    //     "ударять",
    //     "поражать",
    //   ],
    //   en: [
    //     "hit",
    //   ],
    // },
    // {
    //   ru: [
    //     "прятать",
    //     "скрывать",
    //   ],
    //   en: [
    //     "hide",
    //   ],
    // },
    // {
    //   ru: [
    //     "слышать",
    //   ],
    //   en: [
    //     "hear",
    //   ],
    // },
    // {
    //   ru: [
    //     "иметь",
    //     "обладать",
    //   ],
    //   en: [
    //     "have",
    //   ],
    //   pl: [
    //     "mieć",
    //   ],
    // },
    // {
    //   ru: [
    //     "висеть",
    //     "вешать",
    //   ],
    //   en: [
    //     "hang",
    //   ],
    // },
    // {
    //   ru: [
    //     "расти",
    //     "выращивать",
    //     "растить",
    //   ],
    //   en: [
    //     "grow",
    //   ],
    // },
    // {
    //   ru: [
    //     "терять",
    //     "проигрывать",
    //   ],
    //   en: [
    //     "lose",
    //   ],
    // },
    // {
    //   ru: [
    //     "знать (что-то)",
    //     "иметь представление",
    //   ],
    //   en: [
    //     "know",
    //   ],
    //   pl: [
    //     "wiedzieć",
    //   ],
    // },
    // {
    //   ru: [
    //     "класть",
    //     "положить",
    //     "покрывать",
    //   ],
    //   en: [
    //     "lay",
    //   ],
    // },
    // {
    //   ru: [
    //     "вести за собой",
    //     "вести",
    //     "сопровождать",
    //     "руководить",
    //   ],
    //   en: [
    //     "lead",
    //   ],
    // },
    // {
    //   ru: [
    //     "учить",
    //     "учиться",
    //     "изучать",
    //   ],
    //   en: [
    //     "learn",
    //     "study",
    //   ],
    //   pl: [
    //     "uczyć się",
    //   ],
    // },
    // {
    //   ru: [
    //     "покидать",
    //     "оставлять",
    //     "уходить",
    //     "уезжать",
    //   ],
    //   en: [
    //     "leave",
    //   ],
    // },
    // {
    //   ru: [
    //     "давать взаймы",
    //     "одалживать",
    //   ],
    //   en: [
    //     "lend",
    //   ],
    // },
    // {
    //   ru: [
    //     "позволять",
    //     "разрешать",
    //   ],
    //   en: [
    //     "let",
    //     "allow",
    //   ],
    // },
    // {
    //   ru: [
    //     "лежать",
    //     "лгать",
    //   ],
    //   en: [
    //     "lie",
    //   ],
    // },
    // {
    //   ru: [
    //     "зажигать",
    //     "освещать",
    //   ],
    //   en: [
    //     "light",
    //   ],
    // },
    // {
    //   ru: [
    //     "производить",
    //     "делать",
    //   ],
    //   en: [
    //     "make",
    //     "manufacture",
    //     "produce",
    //   ],
    // },
    // {
    //   ru: [
    //     "означать",
    //     "значить",
    //     "иметь в виду",
    //     "подразумевать",
    //   ],
    //   en: [
    //     "mean",
    //   ],
    // },
    // {
    //   ru: [
    //     "встречать",
    //     "знакомиться",
    //   ],
    //   en: [
    //     "meet",
    //   ],
    // },
    // {
    //   ru: [
    //     "быть обязанным",
    //     "быть должным",
    //   ],
    //   en: [
    //     "must",
    //     "have to",
    //   ],
    // },
    // {
    //   ru: [
    //     "платить",
    //     "оплачивать",
    //     "рассчитываться",
    //   ],
    //   en: [
    //     "pay",
    //   ],
    // },
    // {
    //   ru: [
    //     "класть",
    //     "ставить",
    //     "размещать",
    //   ],
    //   en: [
    //     "put",
    //   ],
    // },
    // {
    //   ru: [
    //     "читать",
    //   ],
    //   en: [
    //     "read",
    //   ],
    //   pl: [
    //     "czytać",
    //   ],
    // },
    // {
    //   ru: [
    //     "прочитать",
    //   ],
    //   pl: [
    //     "przeczytać",
    //   ],
    // },
    // {
    //   ru: [
    //     "ехать верхом",
    //     "ездить верхом",
    //     "скакать",
    //   ],
    //   en: [
    //     "ride",
    //   ],
    // },
    // {
    //   ru: [
    //     "звенеть",
    //     "звонить",
    //   ],
    //   en: [
    //     "ring",
    //   ],
    // },
    // {
    //   ru: [
    //     "восходить",
    //     "вставать",
    //     "подниматься",
    //   ],
    //   en: [
    //     "rise",
    //   ],
    // },
    // {
    //   ru: [
    //     "бежать",
    //     "бегать",
    //   ],
    //   en: [
    //     "run",
    //   ],
    // },
    // {
    //   ru: [
    //     "сказать",
    //   ],
    //   en: [
    //     "say",
    //   ],
    // },
    // {
    //   ru: [
    //     "видеть",
    //   ],
    //   en: [
    //     "see",
    //   ],
    // },
    {
      ru: [
        "искать",
        "разыскивать",
      ],
      en: [
        // "seek",
        "look for",
      ],
    },
    // {
    //   ru: [
    //     "продавать",
    //   ],
    //   en: [
    //     "sell",
    //   ],
    // },
    // {
    //   ru: [
    //     "посылать",
    //     "отправлять",
    //   ],
    //   en: [
    //     "send",
    //   ],
    // },
    // {
    //   ru: [
    //     "устанавливать",
    //     "задавать",
    //     "назначать",
    //   ],
    //   en: [
    //     "set",
    //   ],
    // },
    // {
    //   ru: [
    //     "трясти",
    //     "встряхивать",
    //   ],
    //   en: [
    //     "shake",
    //   ],
    // },
    // {
    //   ru: [
    //     "светить",
    //     "сиять",
    //   ],
    //   en: [
    //     "shine",
    //   ],
    // },
    // {
    //   ru: [
    //     "стрелять",
    //     "снимать на камеру",
    //   ],
    //   en: [
    //     "shoot",
    //   ],
    // },
    // {
    //   ru: [
    //     "показывать",
    //   ],
    //   en: [
    //     "show",
    //   ],
    // },
    // {
    //   ru: [
    //     "закрывать",
    //   ],
    //   en: [
    //     "shut",
    //     "close",
    //   ],
    // },
    // {
    //   ru: [
    //     "петь",
    //   ],
    //   en: [
    //     "sing",
    //   ],
    // },
    // {
    //   ru: [
    //     "тонуть",
    //     "погружаться",
    //   ],
    //   en: [
    //     "sink",
    //   ],
    // },
    // {
    //   ru: [
    //     "сидеть",
    //   ],
    //   en: [
    //     "sit",
    //   ],
    // },
    // {
    //   ru: [
    //     "спать",
    //   ],
    //   en: [
    //     "sleep",
    //   ],
    // },
    // {
    //   ru: [
    //     "скользить",
    //   ],
    //   en: [
    //     "slide",
    //   ],
    // },
    // {
    //   ru: [
    //     "пахнуть",
    //     "нюхать",
    //   ],
    //   en: [
    //     "smell",
    //   ],
    // },
    // {
    //   ru: [
    //     "говорить",
    //     "разговаривать",
    //   ],
    //   en: [
    //     "speak",
    //     "talk",
    //   ],
    //   pl: [
    //     "mówić",
    //   ],
    // },
    // {
    //   ru: [
    //     "тратить",
    //     "расходовать",
    //     "проводить (время)",
    //   ],
    //   en: [
    //     "spend",
    //   ],
    // },
    // {
    //   ru: [
    //     "проливать",
    //     "разливать",
    //   ],
    //   en: [
    //     "spill",
    //   ],
    // },
    // {
    //   ru: [
    //     "портить",
    //     "баловать",
    //   ],
    //   en: [
    //     "spoil",
    //   ],
    // },
    // {
    //   ru: [
    //     "стоять",
    //   ],
    //   en: [
    //     "stand",
    //   ],
    // },
    // {
    //   ru: [
    //     "воровать",
    //     "красть",
    //   ],
    //   en: [
    //     "steal",
    //   ],
    // },
    // {
    //   ru: [
    //     "втыкать",
    //     "приклеивать",
    //   ],
    //   en: [
    //     "stick",
    //   ],
    // },
    // {
    //   ru: [
    //     "подметать",
    //     "мести",
    //   ],
    //   en: [
    //     "sweep",
    //   ],
    // },
    // {
    //   ru: [
    //     "плавать",
    //     "плыть",
    //   ],
    //   en: [
    //     "swim",
    //   ],
    // },
    // {
    //   ru: [
    //     "качаться",
    //     "раскачиваться",
    //   ],
    //   en: [
    //     "swing",
    //   ],
    // },
    // {
    //   ru: [
    //     "брать",
    //   ],
    //   en: [
    //     "take",
    //   ],
    //   pl: [
    //     "brać",
    //   ],
    // },
    // {
    //   ru: [
    //     "обучать",
    //     "учить",
    //     "преподавать",
    //   ],
    //   en: [
    //     "teach",
    //   ],
    // },
    // {
    //   ru: [
    //     "рвать",
    //     "разрывать",
    //   ],
    //   en: [
    //     "tear",
    //   ],
    // },
    // {
    //   ru: [
    //     "рассказывать",
    //   ],
    //   en: [
    //     "tell",
    //   ],
    // },
    // {
    //   ru: [
    //     "думать",
    //   ],
    //   en: [
    //     "think",
    //   ],
    // },
    // {
    //   ru: [
    //     "бросать",
    //     "кидать",
    //     "метать",
    //   ],
    //   en: [
    //     "throw",
    //   ],
    // },
    // {
    //   ru: [
    //     "понимать",
    //   ],
    //   en: [
    //     "understand",
    //     "comprehend",
    //   ],
    //   pl: [
    //     "rozumieć"
    //   ],
    // },
    // {
    //   ru: [
    //     "просыпаться",
    //     "будить",
    //   ],
    //   en: [
    //     "wake",
    //   ],
    // },
    // {
    //   ru: [
    //     "носить (одежду)",
    //   ],
    //   en: [
    //     "wear",
    //   ],
    // },
    // {
    //   ru: [
    //     "выигрывать",
    //     "побеждать",
    //   ],
    //   en: [
    //     "win",
    //   ],
    // },
    // {
    //   ru: [
    //     "писать",
    //   ],
    //   en: [
    //     "write",
    //   ],
    //   pl: [
    //     "pisać",
    //   ],
    // },
    // {
    //   ru: [
    //     "написать",
    //   ],
    //   pl: [
    //     "napisać"
    //   ],
    // },
    // {
    //   ru: [
    //     "ждать (чего-то)",
    //   ],
    //   en: [
    //     "wait (for)",
    //   ],
    //   pl: [
    //     "czekać",
    //   ],
    // },
    // {
    //   ru: [
    //     "хотеть",
    //   ],
    //   en: [
    //     "want",
    //   ],
    //   pl: [
    //     "chcieć",
    //   ],
    // },
    // {
    //   ru: [
    //     "гулять",
    //     "прогуливаться",
    //     "идти пешком",
    //     "ходить",
    //     "идти",
    //   ],
    //   en: [
    //     "walk",
    //   ],
    //   pl: [
    //     "chodzić",
    //     "iść",
    //     "spacerować",
    //   ],
    // },
    // {
    //   ru: [
    //     "решать",
    //   ],
    //   en: [
    //     "solve",
    //     "tackle",
    //   ],
    //   pl: [
    //     "decydować",
    //   ],
    // },
    // {
    //   ru: [
    //     "решить",
    //   ],
    //   pl: [
    //     "zdecydować",
    //   ],
    // },
    // {
    //   ru: [
    //     "проживать",
    //     "жить",
    //   ],
    //   en: [
    //     "live",
    //   ],
    //   pl: [
    //     "mieszkać",
    //   ],
    // },
    // {
    //   ru: [
    //     "называть",
    //   ],
    //   en: [
    //     "call",
    //   ],
    //   pl: [
    //     "nazywać",
    //   ],
    // },
    // {
    //   ru: [
    //     "называться",
    //   ],
    //   en: [
    //     "nazywać się",
    //   ],
    // },
    // {
    //   ru: [
    //     "наблюдать",
    //     "следить",
    //     "смотреть (~ телевизор)",
    //   ],
    //   en: [
    //     "watch",
    //   ],
    //   pl: [
    //     "oglądać",
    //   ],
    // },
    // {
    //   ru: [
    //     "слушать (что-то)",
    //   ],
    //   en: [
    //     "listen (to)",
    //   ],
    //   pl: [
    //     "słuchać",
    //   ],
    // },
    // {
    //   ru: [
    //     "послушать",
    //   ],
    //   pl: [
    //     "posłuchać",
    //   ],
    // },
    // {
    //   ru: [
    //     "повторять",
    //   ],
    //   en: [
    //     "repeat",
    //   ],
    //   pl: [
    //     "powtarzać",
    //   ],
    // },
    // {
    //   ru: [
    //     "повторить",
    //   ],
    //   pl: [
    //     "powtórzyć",
    //   ],
    // },
    // {
    //   ru: [
    //     "работать",
    //   ],
    //   en: [
    //     "work",
    //   ],
    //   pl: [
    //     "pracować",
    //   ],
    // },
    // {
    //   ru: [
    //     "спрашивать",
    //     "просить",
    //   ],
    //   en: [
    //     "ask",
    //   ],
    //   pl: [
    //     "pytać",
    //   ],
    // },
    // {
    //   ru: [
    //     "спросить",
    //   ],
    //   pl: [
    //     "zapytać",
    //   ],
    // },
    // {
    //   ru: [
    //     "проверять",
    //   ],
    //   en: [
    //     "check",
    //     "correct",
    //   ],
    //   pl: [
    //     "sprawdzać",
    //   ],
    // },
    // {
    //   ru: [
    //     "проверить",
    //   ],
    //   pl: [
    //     "sprawdzić",
    //   ],
    // },
    // {
    //   ru: [
    //     "заполнять",
    //   ],
    //   en: [
    //     "complete",
    //     "fill",
    //   ],
    //   pl: [
    //     "uzupełniać",
    //   ],
    // },
    // {
    //   ru: [
    //     "вписывать",
    //   ],
    //   pl: [
    //     "wpisywać",
    //   ],
    // },
    // {
    //   ru: [
    //     "вписать",
    //   ],
    //   pl: [
    //     "wpisać",
    //   ],
    // },
    // {
    //   ru: [
    //     "заполнить",
    //   ],
    //   pl: [
    //     "uzupełnić",
    //   ],
    // },
    // {
    //   ru: [
    //     "предпочитать",
    //   ],
    //   en: [
    //     "prefer",
    //   ],
    //   pl: [
    //     "woleć",
    //   ],
    // },
    {
      ru: [
        "размещать",
        "вмещать",
      ],
      en: [
        "accommodate",
      ],
    },
    {
      ru: [
        "взаимодействовать",
      ],
      en: [
        "interact",
      ],
    },
  ],










  "Прилагательные (Базовые прилагательные)": [
    {
      ru: [
        "тихий",
        "спокойный",
      ],
      en: [
        "quiet",
        "calm",
      ],
    },
    {
      ru: [
        "хороший",
      ],
      en: [
        "good",
      ],
      pl: [
        "dobry",
      ],
    },
    {
      ru: [
        "добрый",
      ],
      en: [
        "kind",
      ],
      pl: [
        "dobry",
      ],
    },
    {
      ru: [
        "неофициальный",
      ],
      en: [
        "informal",
      ],
      pl: [
        "nieoficjalny",
      ],
    },
    {
      ru: [
        "официальный",
      ],
      en: [
        "formal",
      ],
      pl: [
        "oficjalny",
      ],
    },
    {
      ru: [
        "универсальный",
      ],
      en: [
        "universal",
      ],
      pl: [
        "uniwersalny",
      ],
    },
    {
      ru: [
        "ложный",
        "ненастоящий",
      ],
      pl: [
        "nieprawdziwy",
      ],
    },
    {
      ru: [
        "правдивый",
        "настоящий",
      ],
      pl: [
        "prawdziwy",
      ],
    },
    {
      ru: [
        "новый",
      ],
      en: [
        "new",
      ],
      pl: [
        "nowy",
      ],
    },
    {
      ru: [
        "подходящий",
        "соответствующий",
      ],
      pl: [
        "odpowiedny",
      ],
    },
    {
      ru: [
        "мужской",
      ],
      en: [
        "male",
      ],
      pl: [
        "męski",
      ],
    },
    {
      ru: [
        "женский",
      ],
      en: [
        "female",
      ],
      pl: [
        "żeński",
      ],
    },
    {
      ru: [
        "агрессивный",
      ],
      pl: [
        "agresywny",
      ],
    },
    {
      ru: [
        "активный",
      ],
      pl: [
        "aktywny",
      ],
    },
    {
      ru: [
        "амбициозный",
        "честолюбивый",
      ],
      pl: [
        "ambitny",
      ],
    },
    {
      ru: [
        "некрасивый",
        "уродливый",
      ],
      pl: [
        "brzydki",
      ],
    },
    {
      ru: [
        "больной",
      ],
      en: [
        "ill",
        "sick",
      ],
      pl: [
        "chory",
      ],
    },
    {
      ru: [
        "элегантный",
      ],
      pl: [
        "elegancki",
      ],
    },
    {
      ru: [
        "эмоциональный",
      ],
      pl: [
        "emocjonalny",
      ],
    },
    {
      ru: [
        "энергичный",
      ],
      pl: [
        "energiczny",
      ],
    },
    {
      ru: [
        "европейский",
      ],
      pl: [
        "europejski",
      ],
    },
    {
      ru: [
        "толстый",
        "полный",
      ],
      pl: [
        "gruby",
      ],
    },
    {
      ru: [
        "другой",
      ],
      pl: [
        "inny",
      ],
    },
    {
      ru: [
        "умный",
        "интеллигентный",
      ],
      pl: [
        "inteligentny",
      ],
    },
    {
      ru: [
        "языковой",
      ],
      pl: [
        "językowy",
      ],
    },
    {
      ru: [
        "креативный",
        "творческий",
      ],
      pl: [
        "kreatywny",
      ],
    },
    {
      ru: [
        "кредитный",
      ],
      pl: [
        "kredytowy",
      ],
    },
    {
      ru: [
        "красивая",
      ],
      pl: [
        "ładna",
      ],
    },
    {
      ru: [
        "медицинский",
      ],
      pl: [
        "medyczny",
      ],
    },
    {
      ru: [
        "милый",
        "приятный",
      ],
      pl: [
        "miły",
      ],
    },
    {
      ru: [
        "молодой",
      ],
      pl: [
        "młody",
      ],
    },
    {
      ru: [
        "национальный",
      ],
      pl: [
        "narodowy",
      ],
    },
    {
      ru: [
        "зубной",
        "стоматологический",
      ],
      pl: [
        "dentystyczny",
      ],
    },
    {
      ru: [
        "испанский",
      ],
      pl: [
        "hiszpański",
      ],
    },
    {
      ru: [
        "японский",
      ],
      pl: [
        "japoński",
      ],
    },
    {
      ru: [
        "следующий",
      ],
      pl: [
        "następny",
      ],
    },
    {
      ru: [
        "натуральный",
        "естественный",
      ],
      pl: [
        "naturalny",
      ],
    },
    {
      ru: [
        "немилый",
        "неприветливый",
      ],
      pl: [
        "niemiły",
      ],
    },
    {
      ru: [
        "неспортивный",
        "не спортивного телосложения",
      ],
      pl: [
        "niewysportowany",
      ],
    },
    {
      ru: [
        "низкий",
      ],
      pl: [
        "niski",
      ],
    },
    {
      ru: [
        "иностранный",
        "заграничный",
      ],
      pl: [
        "obcy",
      ],
    },
    {
      ru: [
        "поданный",
        "предоставленный",
      ],
      pl: [
        "podany",
      ],
    },
    {
      ru: [
        "популярный",
      ],
      pl: [
        "popularny",
      ],
    },
    {
      ru: [
        "правильный",
        "верный",
      ],
      pl: [
        "prawidłowy",
      ],
    },
    {
      ru: [
        "подходящий",
      ],
      pl: [
        "właściwy",
      ],
    },
    {
      ru: [
        "проектный",
      ],
      pl: [
        "projektowy",
      ],
    },
    {
      ru: [
        "красивый(о мужчине)",
      ],
      pl: [
        "przystojny",
      ],
    },
    {
      ru: [
        "рациональный",
      ],
      pl: [
        "racjonalny",
      ],
    },
    {
      ru: [
        "романтичный",
        "романтический",
      ],
      pl: [
        "romantyczny",
      ],
    },
    {
      ru: [
        "сентиментальный",
      ],
      pl: [
        "sentymentalny",
      ],
    },
    {
      ru: [
        "раздраженный",
      ],
      pl: [
        "sfrustrowany",
      ],
    },
    {
      ru: [
        "грустный",
      ],
      pl: [
        "smutny",
      ],
    },
    {
      ru: [
        "спонтанный",
        "импульсивный",
      ],
      pl: [
        "spontaniczny",
      ],
    },
    {
      ru: [
        "среднего роста",
      ],
      pl: [
        "średniego wzrostu",
      ],
    },
    {
      ru: [
        "старый",
      ],
      pl: [
        "stary",
      ],
    },
    {
      ru: [
        "симпатичный",
      ],
      pl: [
        "sympatyczny",
      ],
    },
    {
      ru: [
        "худой",
        "стройный",
        "щуплый",
      ],
      pl: [
        "szczupły",
      ],
    },
    {
      ru: [
        "туристический",
      ],
      pl: [
        "turystyczny",
      ],
    },
    {
      ru: [
        "талантливый",
        "одаренный",
      ],
      pl: [
        "utalentowany",
      ],
    },
    {
      ru: [
        "веселый",
      ],
      pl: [
        "wesoły",
      ],
    },
    {
      ru: [
        "итальянский",
      ],
      pl: [
        "włoski",
      ],
    },
    {
      ru: [
        "высокий",
      ],
      pl: [
        "wysoki",
      ],
    },
    {
      ru: [
        "спортивный",
        "спортивного телосложения",
      ],
      pl: [
        "wysportowany",
      ],
    },
    {
      ru: [
        "здоровый",
      ],
      pl: [
        "zdrowy",
      ],
    },
    {
      ru: [
        "в стрессе",
      ],
      pl: [
        "zestresowany",
      ],
    },
    {
      ru: [
        "стоящий",
      ],
      en: [
        "rewarding",
      ],
    },
    {
      ru: [
        "жесткий",
        "твердый",
      ],
      en: [
        "hard",
      ],
    },
    {
      ru: [
        "профессиональный",
      ],
      en: [
        "professional",
      ],
    },
  ],
  
  
  
  
  
  
  
  
  
  
  "Наречия": [
    {
      ru: [
        "всегда",
      ],
      en: [
        "always",
      ],
    },
    {
      ru: [
        "прекрасно",
        "отлично",
      ],
      pl: [
        "świetnie",
      ],
    },
    {
      ru: [
        "очень хорошо",
      ],
      pl: [
        "bardzo dobrze",
      ],
    },
    {
      ru: [
        "хорошо",
      ],
      pl: [
        "dobrze",
      ],
    },
    {
      ru: [
        "так себе",
      ],
      pl: [
        "tak sobie",
      ],
    },
    {
      ru: [
        "плохо",
      ],
      pl: [
        "źle",
      ],
    },
    {
      ru: [
        "очень плохо",
      ],
      pl: [
        "bardzo źle",
      ],
    },
    {
      ru: [
        "ужасно",
      ],
      pl: [
        "fatalnie",
      ],
    },
    {
      ru: [
        "сейчас",
        "теперь",
      ],
      pl: [
        "teraz",
      ],
    },
    {
      ru: [
        "вслух",
      ],
      pl: [
        "na głos",
      ],
    },
    {
      ru: [
        "куда?",
      ],
      pl: [
        "dokąd?",
      ],
    },
    {
      ru: [
        "прямо сейчас",
      ],
      en: [
        "right now",
      ],
    },
    {
      ru: [
        "сейчас",
        "текущий момент",
        "настоящее время",
      ],
      en: [
        "now",
        "currently",
      ],
    },
    {
      ru: [
        "опять",
        "снова",
      ],
      en: [
        "again",
      ],
    },
  ],










  "Essay": [
    // {
    //   "ru": ["Вопросы, связанные с ..."],
    //   "en": ["Questions related to ..."]
    // },
    // {
    //   "ru": ["(вопросы) часто обсуждаются в (наши дни|сегодня|сейчас)"],
    //   "en": ["(questions) are (frequently|often) discussed nowadays"]
    // },
    // {
    //   "ru": ["(Пока|Хотя) некоторые люди (считают|верят|думают), что ..."],
    //   "en": ["While some people (believe|think) that ..."]
    // },
    // {
    //   "ru": ["другие спорят с этой позицией"],
    //   "en": ["others argue with this position"]
    // },
    // {
    //   "ru": ["Лично я (думаю|верю|считаю), что ..."],
    //   "en": ["Personally, I (think|believe) that ..."]
    // },
    // {
    //   "ru": ["В этом эссе будет подробно обсуждаться эта тема (до того|перед тем), как сделать вывод."],
    //   "en": ["This essay will discuss the topic in details prior to drawing a conclusion."]
    // },
    // {
    //   "ru": [
    //     "без сомнения ...",
    //     "нет сомнений, что ...",
    //     "без сомнения что ...",
    //     "нет сомнений в том что ...",
    //     "не вызывает сомнений в том что ...",
    //   ],
    //   "en": ["there is no doubt that ..."]
    // },
    // {
    //   "ru": [
    //     "в первую очередь",
    //     "на первом месте",
    //     "во-первых",
    //     "прежде всего",
    //   ],
    //   "en": [
    //     "in the first place",
    //     "firstly",
    //     "first of all",
    //   ]
    // },
    // {
    //   "ru": ["также (ясно|понятно), что ..."],
    //   "en": ["also, it is clear that ..."]
    // },
    // {
    //   ru: [
    //     "в тоже время, ...",
    //     "вместе с тем, ...",
    //   ],
    //   en: ["at the same time, ..."]
    // },
    // {
    //   ru: ["также следует (указать|сказать|обозначить), что ..."],
    //   en: ["it should also be stated that ..."],
    // },
    // {
    //   ru: [
    //     "дополнительно, ...",
    //     "кроме того, ...",
    //     "более того, ...",
    //     "вдобавок, ...",
    //   ],
    //   en: [
    //     "additionally, ...",
    //     "in addition, ...",
    //     "moreover, ...",
    //     "besides, ...",
    //     "furthermore, ...",
    //   ],
    // },
    // {
    //   ru: ["В заключение я (считаю|верю|думаю), что"],
    //   en: ["In conclusion, I (believe|think) that"],
    // },
    // {
    //   ru: ["После анализа (предмета|темы) стало очевидно, что ..."],
    //   en: ["After analyzing the (subject|topic), it has become evident that ..."],
    // },
    // {
    //   ru: ["некоторые люди (считают|верят|думают) что ..."],
    //   en: ["some people (believe|think) that ..."],
    // },
    // {
    //   ru: ["Я согласен, что ..."],
    //   en: ["I agree that ..."],
    // },
    // {
    //   ru: ["технологии сильно улучшились"],
    //   en: ["technology has improved (dramatically|extremely)"],
    // },
    // {
    //   ru: ["есть (несколько|ряд) преимуществ ... (чего-то)"],
    //   en: ["there are (a number of|some) benefits of ... (sth)"],
    // },
    // {
    //   ru: ["(что-то) имеет положительный эффект на ..."],
    //   en: ["(sth) has a positive effect on ..."],
    // },
    // {
    //   ru: ["положительные (эффекты|стороны) (чего-то)",],
    //   en: ["the positive effects of ..."],
    // },
    // {
    //   ru: ["(что-то) перевешивается (рядом недостатков|несколькими недостатками)"],
    //   en: ["(sth) is outweighed by (a number of|some) drawbacks"],
    // },
    // {
    //   ru: ["(Кто-то) может получить (выгоду|преимущество) от ..."],
    //   en: ["(Smbd) can benefit from ..."],
    // },
    // {
    //   ru: ["(Что-то) представляет угрозу для ... (здоровье, общество, ...)"],
    //   en: ["(Something) poses a threat to ... (one's health, society, ...)"]
    // },
    // {
    //   ru: ["(Что-то) чрезвычайно важно для ..."],
    //   en: ["(Something) is (extremely|dramatically) important for ..."],
    // },
    // {
    //   ru: ["решать проблему"],
    //   en: ["to (solve|tackle) the (problem|issue)"],
    // },
    // {
    //   ru: [
    //     "(ряд|несколько) (научно-исследовательских проектов|исследований) показали, что ...",
    //     "некоторые (научно-исследовательский проекты|исследования) показали, что ..."
    //   ],
    //   en: ["(a number of|some) scientific research projects have demonstrated that ..."],
    // },
    // {
    //   ru: ["например"],
    //   en: [
    //     "for example, ...",
    //     "for instance, ...",
    //   ],
    // },
    // {
    //   ru: ["такой как ..."],
    //   en: ["such (as|us) ..."],
    // },
    // {
    //   ru: ["причина (чего-то)"],
    //   en: [
    //     "reason for (sth)",
    //     "cause of (sth)",
    //   ],
    // },
    // {
    //   ru: ["(основная|главная) причина проблемы это ..."],
    //   en: ["the main (reason for|cause of) the (issue|problem) is that ..."],
    // },
    // {
    //   ru: [
    //     "хотя ...",
    //     "несмотря на ...",
    //     "если бы даже ...",
    //     "несмотря на то, что ...",
    //   ],
    //   en: ["although ..."],
    // },
    // {
    //   ru: ["however ..."],
    //   en: [
    //     "однако ...",
    //     "тем не менее ...",
    //     "впрочем ...",
    //   ],
    // },
    // {
    //   ru: [
    //     "следовательно",
    //     "таким образом",
    //   ],
    //   en: [
    //     "consequently",
    //     "therefore",
    //     "so",
    //   ],
    // },
    // {
    //   ru: [
    //     "в результате",
    //   ],
    //   en: [
    //     "as a result",
    //   ],
    // },
    // {
    //   ru: [
    //     "без сомнений, ...",
    //     "несомненно, ...",
    //     "определенно, ...",
    //     "конечно ...",
    //   ],
    //   en: [
    //     "undoubtedly",
    //     "definitely",
    //     "certainly",
    //     "of course",
    //     "sure",
    //   ],
    // },
    // {
    //   ru: ["несмотря на это ..."],
    //   en: ["despite this ..."],
    // },
    // {
    //   ru: ["с одной стороны, ..."],
    //   en: ["on the one hand, ..."],
    // },
    // {
    //   ru: ["с другой стороны, ..."],
    //   en: ["on the other hand, ..."],
    // },
    // {
    //   ru: ["аналогично"],
    //   en: ["likewise"],
    // },
    // {
    //   ru: ["(что-то) необходимо для ..."],
    //   en: ["(sth) is essential for ..."],
    // },
    // {
    //   ru: [
    //     "потому что",
    //     "так как"
    //   ],
    //   en: ["because"],
    // },
    // {
    //   ru: ["из-за"],
    //   en: [
    //     "due to (~ the fact)",
    //     "because of ...",
    //   ],
    // },
    // {
    //   ru: ["влияние (чего-то) на (что-то)"],
    //   en: [
    //     "the impact of (sth) on (sth)",
    //     "the influence of (sth) on (sth)",
    //   ],
    // }
  ],
  
  
  
  
  
  
  
  
  
  
  "Speaking": [
    // {
    //   "ru": [
    //     "Хочешь верь, хочешь нет, ...",
    //     "Хотите верьте, хотите нет, ...",
    //   ],
    //   "en": ["Believe it or not, ..."]
    // },
    // {
    //   "ru": [
    //     "Ты можешь не верить этому, но ...",
    //     "Ты можешь этому не верить, но ...",
    //     "Вы можете не верить этому, но ...",
    //     "Вы можете этому не верить, но ...",
    //   ],
    //   "en": ["You may not believe it, but ..."]
    // },
    // {
    //   "ru": ["Это может звучать странно, но ..."],
    //   "en": ["It may sound strange, but ..."]
    // },
    // {
    //   "ru": ["Удивительная вещь, ..."],
    //   "en": ["The surprising thing is ..."]
    // },
    // {
    //   "ru": [
    //     "Удивительно, ...",
    //     "Как ни странно, ...",
    //     "Довольно странно, ..."
    //   ],
    //   "en": [
    //     "Surprisingly, ...",
    //     "Funnily enough, ...",
    //     "Oddly enough, ..."
    //   ]
    // },
    // {
    //   "ru": [
    //     "В общем-то",
    //     "В общем",
    //     "В общем и целом",
    //     "В целом",
    //     "как правило"
    //   ],
    //   "en": [
    //     "Generally",
    //     "By and large",
    //     "On the whole"
    //   ]
    // },
    // {
    //   "ru": [
    //     "As a rule",
    //     "Normally",
    //     "Usually"
    //   ],
    //   "en": [
    //     "Как правило",
    //     "Как обычно",
    //     "Обычно",
    //     "Нормально"
    //   ]
    // }
    // {
    //   ru: [
    //     "успокойтесь",
    //     "помолчите",
    //   ],
    //   en: [
    //     "be quiet",
    //   ],
    // },










    /* Idioms */
    // {
    //   "ru": [
    //     "проще простого",
    //     "очень просто",
    //   ],
    //   "en": ["a piece of cake"],
    // }










    /* Выражения общего плана */
    // {
    //   "ru": [
    //     "получать удовольствие от ..."
    //   ],
    //   "en": [
    //     "derive pleasure from ..."
    //   ],
    // },
    // {
    //   ru: [
    //     "Это очень мило с вашей стороны",
    //     "Это очень любезно с вашей стороны",
    //   ],
    //   en: [
    //     "That is very kind of you"
    //   ],
    // },
    // {
    //   ru: [
    //     "иметь проблему",
    //     "столкнуться с проблемой",
    //   ],
    //   en: [
    //     "have a problem",
    //   ],
    // },
    // {
    //   ru: [
    //     "приходить (куда-то) с опозданием",
    //     "опаздывать",
    //   ],
    //   en: [
    //     "come (somewhere) late",
    //   ],
    // },
  ],
};